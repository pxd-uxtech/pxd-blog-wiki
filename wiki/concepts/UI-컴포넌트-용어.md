...
UI 설계에서 자주 쓰이지만 혼동되는 용어로 Granularity(입자감)와 Coherence(일체감)가 있다. Granularity는 UI/UX의 정밀도 수준을 나타내는 개념으로, 서비스의 성격과 사용자(퍼소나)에 맞는 정보 표시 및 조작의 세밀함을 의미한다. 너무 세밀하거나 너무 큰 입자감 모두 사용성 문제를 일으킨다. Coherence는 consistency(반복적 일관성)와 구별되는 개념으로, 개별 기능 결정이 아닌 전체 경험이 하나의 느낌으로 통일되는 '일체감'을 뜻한다. 퍼소나와 브랜드 에센스를 기준으로 전체 결정을 연결할 때 비로소 coherent한 디자인이 만들어진다.
