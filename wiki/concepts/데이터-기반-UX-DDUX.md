...
domain: "UX 방법론"
name: "데이터 기반 UX (DDUX)"
sources: ["/1545", "/1201", "/1456", "/1430", "/1389", "/1539", "/1203", "/1244"]
updated: "2026-08-28"
---
# 데이터 기반 UX (DDUX)
...
류정우가 **HCI KOREA 2019**에서 발표한 "Data-Driven UX" 세션은 앞서 다룬 세 가지 관점—사용자 모델링·근거 있는 디자인·성과측정—을 실제 프로젝트 사례로 뒷받침한다.

**데이터 기반 사용자 모델링: True Balance의 '단골 에이전트'**: 인도의 핀테크 스타트업 **Balance Hero**가 2014년부터 운영해 온 통신비 충전 서비스 True Balance는 정성 조사를 통해 주변 지인의 잔액 충전을 대행해주는 '단골 에이전트' 사용자군을 발견했다. 이를 데이터로 검증한 결과 실제 단골 에이전트 비중은 3.4%에 불과했지만, 단골 에이전트가 될 가능성이 있는 사용자는 14.8%로 훨씬 컸다. 정성 조사가 발견한 흥미로운 사용자 유형을 데이터가 정량적으로 뒷받침하고 잠재 타겟 규모까지 제시한 사례로, 정성·정량 조사가 상호 보완적이라는 DDUX의 관점을 보여준다.
...
이어 소개된 **MUJI·WISH·Oisix·Dentsu** 네 글로벌 기업의 사례는 데이터 활용이 실제 비즈니스 성과로 이어진 궤적을 보여준다. MUJI는 웹·앱·매장에 흩어진 회원 데이터를 MUJI PASSPORT 앱으로 통합해, 매장 방문 시점의 재고·매출 히스토리를 근거로 개인화 쿠폰을 자동 발급하는 체계를 만들어 매출 46% 증가와 쿠폰 활용률 100% 증가를 이끌어냈다. WISH는 Treasure Data 아키텍처로 페이스북·앱·웹 클릭스트림을 통합하고, 지역 최적화의 함정에 빠지기 쉬운 A/B 테스트를 반복 자동화한 프레임(perimeter)으로 보완해 일 34억 이벤트를 처리하며 개인화된 쇼핑 경험을 제공했다. Oisix는 유입 경로·행동 로그·클레임 데이터를 바탕으로 머신러닝 이탈 예측 모델을 만들어 케어콜·포인트 지급 같은 대응으로 해지율을 절반으로 낮췄다. Dentsu는 1st·2nd·3rd party 데이터를 결합하는 애드테크 방식을 취했고, 정해진 주기로 도는 **배치 분석**과 특정 이슈를 꼬리 질문으로 파고드는 **애드혹 분석**을 구분해 활용하며 애드혹 분석 비중이 늘어나는 추세를 짚었다.

강연 후반부는 데이터 활용 시 고려해야 할 관점들을 정리한다. 인사이트·트렌드 같은 단어가 수치화된 가치로 바로 이어지지 못하는 경우가 많다는 문제의식에서, "데이터에서 가치를 찾기"보다 **"가치에서 필요한 데이터를 설계하기"**가 더 실용적이라는 관점을 제시한다. 데이터 분석과 직관 중 무엇을 따를지는 뛰어난 직관을 가진 사람의 인사이트를 모델링할 수 있는지가 기준이 되며, 이종 데이터를 결합할 때는 Join·Slice & Dice·Drill down & up 개념을 함께 고려해야 하나의 축에 치우친 결론을 피할 수 있다고 강조한다. 데이터 기반 커뮤니케이션 문화를 조직에 정착시키려면 거창한 담론보다 **작은 성공을 빨리 만들어내는 것**이 중요하다는 실무적 조언으로 마무리된다.

## Alibaba UCAN Conference 2017 — User+Context Modeling

전성진은 2017년 4월 중국 항저우에서 열린 알리바바 그룹의 **UCAN 컨퍼런스**에 키노트 스피커로 초청되어 "User+Context Modeling in UX Design Process"라는 제목으로 발표했다. UCAN은 알리바바 UED Committee가 매년 여는 실무자 중심 비영리 행사로, 그해는 "Neo-Design, Neo-Business"를 주제로 마윈이 제시한 "The Five New Trends"(유통·제조·금융·기술·자원 영역의 A.I. 중심 변화)의 연장선에서 기획됐다.

발표는 소수 사용자 표본에 의존하는 기존 정성적 리서치만으로 사용자 가설을 수립·모델링할 때의 한계를, 사용자의 정량적 데이터로 보완하는 프로세스를 다뤘다. 스마트폰 중심 모바일 환경 덕분에 시간·위치·서비스 이용 데이터 등 사용자 행동에 관한 정보를 과거보다 풍부하게 수집할 수 있게 됐고, 이를 통해 행동 패턴을 추출해 사용자 가설을 탄탄히 뒷받침할 수 있다는 것이 핵심 메시지였다. UX 디자이너가 사용자의 정량 데이터에 관심을 갖고 직접 다룰 수 있어야 한다는 주장은, DDUX의 "인터랙션은 곧 증거(Data)"라는 마인드셋과 같은 문제의식을 공유한다. 같은 발표는 두 달 뒤인 2017년 6월 한국인터넷전문가협회(KIPFA) 주최 "2017 Creative Director Seminar & Recruiting"에서 "Data Driven User+Context Modeling in UX Design Process"라는 제목으로 재발표됐다.
...
- Alibaba UCAN Conference 2017(전성진): "User+Context Modeling in UX Design Process" 발표 — 정성 조사 기반 사용자 가설·모델링을 정량 데이터로 보완하는 프로세스 제안
...
- [[중국 IT 대기업 문화와 디자인 트렌드]] — UCAN 컨퍼런스를 주최한 알리바바 등 중국 IT 기업의 디자인 조직 문화
...
- [HCI KOREA 2019 참관 후기 (2/3)](https://story.pxd.co.kr/1389) — 2019-03-04, 류정우 (작성자)
- [디지털 전환시대의 UXer의 파워는 어디에서 나오는가?](https://story.pxd.co.kr/1539) — 2020-08-05, 전성진
- [[pxd talks 71] 데이터 활용 글로벌 혁신 성공에서 찾아보는 데이터 기반 UX 전문가가 나아갈 길](https://story.pxd.co.kr/1203) — 2017-04-07, 김수연 (2016년 pxd 인턴 작성)
- [2017 Alibaba UCAN Conference 발표 후기](https://story.pxd.co.kr/1244) — 2017-07-20, 전성진
