...
저자는 다른 글에서 NBU 개념이 구글 내부에서 어떻게 조직·자료화되어 있는지도 소개한다. 구글은 2016년 2월 싱가포르에 동남아시아 전담 **NBU 팀**을 신설했는데, 이는 구글이 특정 지역만을 위해 팀을 만든 첫 사례였다. 비슷한 시기 페이스북도 2015년 10월부터 매주 화요일 오전 전 직원이 강제로 매우 느린 네트워크 환경을 체험하는 **2G Tuesdays**를 시행해, 저사양 환경 사용자를 위한 Facebook Lite를 이미 출시했음에도 본사 직원들이 그 불편을 직접 체감하도록 했다. 구글 디자인 라이브러리는 NBU 관련 읽을거리를 모아 제공하는데, 그중 관광투어형 리서치와 대비되는 심층 현지 조사 기법인 **Immersion Trip**을 소개하는 글이 흥미로운 자료로 꼽힌다(같은 문제의식은 얀 칩체이스의 『관찰의 힘』을 다루는 [[현장 관찰 리서치 방법론]]에서 더 자세히 볼 수 있다). 구글 개발자 가이드의 "수십억 사용자를 위한 앱 빌드" 섹션은 더 구체적인 기술 지침을 제공한다. 이미지는 PNG·JPG보다 용량이 작으면서 화질은 더 좋은 **WebP** 포맷을 쓰고, 앱이 네트워크 상태를 감지해 오프라인에서도 가능한 최대한 동작하도록 설계할 것을 권고한다.
...
- 구글 **NBU 팀**(2016년 싱가포르 신설, 특정 지역 전담 첫 사례)과 페이스북 **2G Tuesdays**(전 직원 저속 네트워크 강제 체험) — 글로벌 기업들의 조직적 대응
- 구글 개발자 가이드 권고: 이미지는 PNG/JPG 대신 **WebP** 사용, 오프라인에서도 최대한 동작하도록 네트워크 상태 감지 설계
...
- [[현장 관찰 리서치 방법론]] — 관광투어형 리서치를 지양하는 Immersion Trip과 문제의식을 공유하는 심층 현지 조사 기법
...
- [구글 - Next Billion Users Initiative](https://story.pxd.co.kr/1270) — 2018-01-18, 이 재용
