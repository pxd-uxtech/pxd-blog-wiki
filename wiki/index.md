# pxd 블로그 개념 위키

LLM이 pxd 블로그 글을 읽고 정리한 개념 페이지 모음.

## UX 이론
- [거울 뉴런과 공감적 모방](concepts/거울-뉴런과-공감적-모방.md) — 마르코 야코보니 『미러링피플』: 타인의 행동을 모사해 이해·학습·공감을 형성하는 거울 뉴런 메커니즘, 목표 지향적 모방과 체화된 인지의 연결
- [게슈탈트 법칙과 시각적 통일성](concepts/게슈탈트-법칙과-시각적-통일성.md) — 통일성(Unity)은 디자이너 요소들 사이에 시각적·형태적 유사성이 존재해 보는 이가 조화, 일치, 일관성을 느끼는 것
- [게이미피케이션](concepts/게이미피케이션.md) — MDA 프레임워크·플레이어 유형·라프코스터의 재미이론(재미는 학습에서 온다)으로 본 게이미피케이션 설계 원리
- [기호학과 인덱스](concepts/기호학과-인덱스.md) — 기호의 세 유형(아이콘·인덱스·심벌) 중 대상체와 인과적 연결을 갖는 인덱스(Index)의 개념과 UI 적용
- [디자인 주도 혁신](concepts/디자인-주도-혁신.md) — 로베르토 베르간티의 Design-Driven Innovation: 사용자 관찰 대신 의미(meaning) 재정의를 통한 혁신 접근
- [디자인의 정의와 본질](concepts/디자인의-정의와-본질.md) — 스티브 잡스의 "Design is how it works": 디자인은 겉모습이 아닌 작동 방식과 제품의 본질적 영혼을 표현하는 것
- [디자인 패러다임의 시대적 진화](concepts/디자인-패러다임의-시대적-진화.md) — 이건표 교수의 pxd talks 35: 전산업(Craftsman/Bodyware)→산업화(Stylist)→정보화(Observer/Useware)→드림소사이어티(Facilitator/Cultureware) 4단계 디자인 패러다임, Process 3.0과 미래 디자이너의 Empowerer 역할
- [Desire Path와 거꾸로 UX](concepts/Desire-Path와-거꾸로-UX.md) — 공원 잔디밭 오솔길과 pxd 컵 보관함 사례로 본 Desire Path, 의도와 다른 사용자 행동을 규제가 아닌 숨겨진 니즈의 신호로 보는 거꾸로 UX 사고방식
- [ISO UI 표준](concepts/ISO-UI-표준.md) — ISO 9241·13407·9241-210으로 이어지는 사용성과 UX의 국제 표준 계보
- [Jobs to Be Done](concepts/Jobs-to-be-Done.md) — 특정 상황에서 고객이 해결해야 할 근본 문제(Job)를 중심으로 혁신 기회를 발견하는 클레이튼 크리스텐슨 기반 방법론
- [Socializing을 유도하는 제품 디자인](concepts/Socializing을-유도하는-제품-디자인.md) — Void watch·네스카페 캡슐 머신·메신저 프로필처럼 본연의 기능을 넘어 대화를 유발하는 부가적 스토리를 심는 제품·UI 설계 원칙
- [UI 근본 문제와 UX 핵심 4단계](concepts/UI-근본-문제와-UX-핵심-4단계.md) — 인터페이스 대역폭과 인간 주의력이라는 두 제약자원의 배분 문제로 본 UI, 욕망→반복의 재미→통제감→다시 오는 욕망의 4단계 순환 모델로 본 UX
- [UX 5단계 요소 모델](concepts/UX-5단계-요소-모델.md) — 휴 더벌리의 Strategy→Scope→Structure→Skeleton→Surface 5단계 UX iceberg 모델, Co-UX(M2M vs Man to Man) 구분, 혁신 확산 이론 기반 은행 채널 세분화
- [UX 디자이너를 위한 20가지 원칙](concepts/UX-디자이너를-위한-20가지-원칙.md) — Whitney Hess가 정리하고 전성진이 실무 해석을 덧붙인 UX 디자이너 5대 핵심 가이드와 20가지 세부 원칙, "능력 있고 상냥한 좋은 사람"이라는 인성적 결론
- [마음의 구조와 몰입 4단계](concepts/마음의-구조와-몰입-4단계.md) — pxd talks 26(김한수) 강연: 생각·감정·오감으로 본 마음의 구조와 초의식, 3단계(의지적 몰입)와 4단계(물아일체)로 나뉘는 몰입(Flow)의 진입법
- [메타인지와 디자인 학습](concepts/메타인지와-디자인-학습.md) — 존 플라벨의 메타인지(사고에 대한 사고) 개념과 기획-점검-조절 3단계, 개인·팀(시니어-주니어) 차원의 디자인 학습 실천법
- [밀러의 매직 넘버](concepts/밀러의-매직-넘버.md) — 1956년 조지 밀러가 발표한, 인간의 단기 기억 용량은 7±2개라는 법칙
- [모델 휴먼 프로세서](concepts/모델-휴먼-프로세서.md) — 인간의 지각·인지·운동 시스템으로 구성된 1983년 Card·Moran·Newell의 인지 모델
- [사용자 중심 디자인](concepts/사용자-중심-디자인.md) — UCD(사용자 요구 반영)와 HCD(인간 인지·신체 특성 반영)의 차이, 제프 래스킨과 도널드 노먼의 관점
- [심리학 실험과 UX](concepts/심리학-실험과-UX.md) — 스키너·밀그램·할로·페스팅거·로프터스 등 역사적 심리학 실험이 UX 설계에 주는 이론적 함의
- [어포던스](concepts/어포던스.md) — 디자이너 사이에서 가장 자주 쓰이는 전문 용어 중 하나이지만, 정작 정의가 혼란스러운 개념
- [마우스 커서와 어포던스](concepts/마우스-커서와-어포던스.md) — 버튼·체크박스·라벨의 커서 모양 정책을 어포던스 이론과 웹 표준 관점에서 분석
- [에릭 리스의 사용성 프레임워크](concepts/에릭-리스의-사용성-프레임워크.md) — Euro IA 2013 워크숍 "Usable Usability": 사용성을 물리적 층위(Ease of Use = Functional+Responsive+Ergonomic+Convenient+Foolproof)와 심리적 층위(Elegance and Clarity = Visible+Understandable+Logical+Consistent+Predictable)로 나눈 체크리스트 프레임워크
- [웨인쉔크의 UX 심리 원칙](concepts/웨인쉔크의-UX-심리-원칙.md) — 수잔 웨인쉔크가 정리한 UX 심리학 7가지 원칙: 점진적 공개·사회적 검증·변화맹·원시뇌/감성적 뇌 등 인간 인지·행동 한계 기반 설계 원리
- [지각심리학과 UI 설계](concepts/지각심리학과-UI-설계.md) — 경험·맥락·목표 편향, 중심와·주변시 구조, 안구 도약운동(Saccade)·응시(Fixation)로 UI 가이드라인의 원리를 설명하는 지각심리학 토대
- [진화심리학과 디자인](concepts/진화심리학과-디자인.md) — 대칭성·평균성·아기 스키마 등 진화심리학으로 설명하는 아름다움과 귀여움의 원리
- [초협력자와 협력의 진화](concepts/초협력자와-협력의-진화.md) — 마틴 노왁의 《초협력자》: 진화의 세 번째 규칙 협력, 포괄적합도(rb>c)와 직접·간접 상호성 등 협력의 진화 메커니즘
- [터치 스크롤 UI와 촉각적 멘탈 모델](concepts/터치-스크롤-UI와-촉각적-멘탈-모델.md) — 스크롤바 제거로 콘텐츠를 실체처럼 느끼게 하는 아이폰 스크롤의 본능적(visceral) 멘탈 모델, bounce-back·flicking 등 물리 법칙을 흉내 낸 촉각적 UI 설계
- [통계 프레이밍과 고객 경험 은폐](concepts/통계-프레이밍과-고객-경험-은폐.md) — 스티브 잡스의 아이폰4 안테나게이트 위기 관리 사례로 본, 평균화된 통계 숫자로 개별 고객의 실제 경험을 희석시키는 커뮤니케이션 패턴과 정성 조사 우선의 근거
- [튜링 테스트와 인간다움](concepts/튜링-테스트와-인간다움.md) — 브라이언 크리스천의 『The Most Human Human』: 튜링 테스트와 Most Human Human상, Phonagnosia·Eudaimonia·마인드 업로딩으로 본 인간다움의 조각들
- [프록세믹스와 공간 인류학](concepts/프록세믹스와-공간-인류학.md) — 에드워드 홀의 공간 인류학 이론: 인간의 거리 4단계(밀접·개인·사회·공적)와 사회구심적·사회원심적 공간 개념
- [포그 행동 모델](concepts/포그-행동-모델.md) — BJ 포그의 B=MAT 공식(Behavior=Motivation×Ability×Trigger), Behavior Grid와 Priority Mapping, 습관 형성 곡선으로 행동을 설계하는 모델
- [포카요케](concepts/포카요케.md) — 도요타 품질 기법을 UX에 적용한 오류 방지 설계 원칙
- [피츠의 법칙](concepts/피츠의-법칙.md) — 1954년 폴 피츠가 발표한 운동 행동 모델로, 목표 영역을 클릭하는 데 걸리는 시간 예측
- [행동경제학과 UX](concepts/행동경제학과-UX.md) — 전망 이론·손실 회피·앵커링 등 행동경제학 개념의 UX 설계 적용
- [행동심리학과 강화이론](concepts/행동심리학과-강화이론.md) — 스키너의 조작적 조건화와 강화 스케줄, UX 게이미피케이션과 중독성 패턴의 이론적 배경
- [훅 모델과 습관 형성 디자인](concepts/훅-모델과-습관-형성-디자인.md) — 니르 이얄의 훅 모델 4단계(계기·행동·가변적 보상·투자)와 B=MAT 공식, 진통제·비타민 프레임으로 본 습관 형성 상품 설계
- [힉의 법칙](concepts/힉의-법칙.md) — 선택지 수가 늘어날수록 결정 시간이 증가한다는 법칙
- [미디어 방정식과 CASA](concepts/미디어-방정식과-CASA.md) — 사람은 미디어를 실제 사람처럼 대한다 — 무의식적 사회적 반응의 원리
- [문화심리학과 UX](concepts/문화심리학과-UX.md) — 리처드 니스벳의 동서양 사고 차이와 UX 설계 함의
- [컬처코드와 무의식적 문화 코드](concepts/컬처코드와-무의식적-문화-코드.md) — 클로테르 라파이유의 각인 발견 방법론과 문화 코드 5원칙
- [단순함의 법칙](concepts/단순함의-법칙.md) — 존 마에다의 10가지 법칙: 축소·조직·시간·학습·차이·문맥·감성·신뢰·실패·하나 — 명백한 것을 제거하고 의미 있는 것만 더하는 설계 철학
- [내재적 동기와 자기결정성 이론](concepts/내재적-동기와-자기결정성-이론.md) — 다니엘 핑크 『드라이브』의 동기 1.0→2.0→3.0 발전, 데시·라이언의 자기결정성이론(자율성·숙련·목적), 애머빌의 창의성 내재동기 원리
- [체화된 인지와 확장된 마음](concepts/체화된-인지와-확장된-마음.md) — 앤디 클락의 생태적 조합·인식적 행동·인지 적소 구축 이론과 형태적 계산, 래리 테슬러의 복잡성 보존의 법칙·켄트 벡의 디자인 정의로 본 뇌-몸-환경 확장 인지
- [TRIZ와 발명적 문제해결](concepts/TRIZ와-발명적-문제해결.md) — 겐리히 알츠슐러의 특허 분석 기반 발명 이론: 기술적/물리적 모순 구분과 시간·공간·전체-부분 분리 전략으로 최적해 아닌 이상해를 찾는 문제해결 프레임워크
- [브랜드 이해와 크리에이티브 전략](concepts/브랜드-이해와-크리에이티브-전략.md) — 아웃소싱·인하우스 디자이너의 관점 차이, 브랜드 플랫폼 4단계와 공감대 형성을 위한 8가지 크리에이티브 전략(Observation·Brand Essence·Storytelling·Positioning 등)
- [전략적 UX 디자인](concepts/전략적-UX-디자인.md) — 개별 화면·기능 단위 결정을 사업 전략과 연계하는 접근, UCD·사용성평가·더블다이아몬드·퍼소나를 customer insight 단계에 맞춰 활용하는 4가지 실행 방법
- [전략적 직관](concepts/전략적-직관.md) — 윌리엄 더건의 『전략적 직관』이론: 혁신은 창의력이 아니라 과거 조각들의 우연적 재조합에서 비롯되며, 어피니티 다이어그램·퍼소나가 혁신적인 이유도 데이터의 우연적 재배열에 있다는 관점
- [혁신 신화와 오해](concepts/혁신-신화와-오해.md) — 스콧 버쿤의 『이노베이션 신화의 진실과 오해』: 에피파니 신화·외로운 이노베이터 신화 반박, 오스본 브레인스토밍 4원칙과 로저스 혁신 수용 속도 5요인

## UX 방법론
- [UX 라이팅](concepts/UX-라이팅.md) — 디지털 서비스 내 모든 텍스트를 사용자 관점에서 설계하는 분야
- [UX 리서치 임팩트 관리](concepts/UX-리서치-임팩트-관리.md) — UX 리서치가 조직에 미친 영향을 Tao Dong의 3레벨 모델로 체계적으로 추적·관리하는 방법
- [UX 리서치 통계 분석](concepts/UX-리서치-통계-분석.md) — UX 리서치에서 수집한 숫자 지표를 통계적으로 해석하는 방법
- [UX 리서치 유형](concepts/UX-리서치-유형.md) — 발견적 리서치(Discovery)와 평가적 리서치(Evaluative)의 구분, pxd의 모델링 중심 리서치 철학
- [ResearchOps](concepts/ResearchOps.md) — Kate Towsey의 리서치 조직적 영향력 극대화 개념, UXMC 2021 세션에서 제시된 데이터 통합·경영진 후원·점진적 확장 등 ResearchOps 실천 10가지 방향
- [UX 연출과 모션 디자인 원칙](concepts/UX-연출과-모션-디자인-원칙.md) — 디즈니 애니메이션 12원칙 기반 UX 모션 설계: 피드백·피드포워드·공간인식·사용자포커스·브랜드톤의 5원칙
- [UX 정량 평가](concepts/UX-정량-평가.md) — 참가자 수 결정 기준·표본 선정·신뢰 구간 해석으로 사용자 경험을 수치화해 의사결정 근거를 확보하는 방법론
- [UX 현지화 전략](concepts/UX-현지화-전략.md) — 글로벌 서비스를 위한 현지화(L10N)와 국제화(I18N) 전략, 맥락·기술·언어·법규 네 가지 고려 영역
- [Future Conference 2018과 UX 디자이너의 역할](concepts/Future-Conference-2018과-UX-디자이너의-역할.md) — Addy Lee(구글)의 파트너십 3원칙, 최지호(네이버 웨일)의 조사 없는 UX 디자인과 '파급력 우선' 전환, Jason Kim(인스타그램)의 미션 기반 Discover-Design-Deliver 프로세스
- [Future Conference 2019와 글로벌 프로덕트 UX](concepts/Future-Conference-2019와-글로벌-프로덕트-UX.md) — 구글 뉴스의 멘탈모델·개인화·상태인식 3축 전략과 그랩(Grab)의 국가별 드라이버 멘탈 모델 차이 대응, 관찰-정의-전략-프로토타입-실험 5단계 디자인 프로세스
- [IDG Business Impact & Big Data 컨퍼런스](concepts/IDG-Business-Impact-Big-Data-컨퍼런스.md) — 이마트 오미현의 우수고객 관리→마이크로 트렌드 CRM 전환 사례와 넷플릭스 조슈아 팍스의 실시청 데이터 기반 개인화 추천·포스터 알고리즘
- [디지털 마케팅 컨퍼런스 2018](concepts/디지털-마케팅-컨퍼런스-2018.md) — AKQA·JWT·크리테오 발표로 본 데이터 해석과 맥락, 소셜 크레딧 시스템·프로슈머·매스 개인화(Mass personalization) 개념
- [Euro IA 2016 컨퍼런스](concepts/Euro-IA-2016-컨퍼런스.md) — 네덜란드 암스테르담 Euro IA 2016 "Connected Things Amongst Us" 참관기, BBC UX팀의 제이크 냅 오리지널 5일 디자인 스프린트 워크숍과 팀 역량 갭 매핑 워크숍
- [Interaction16 컨퍼런스](concepts/Interaction16-컨퍼런스.md) — IxDA 주최 2016년 핀란드 헬싱키 컨퍼런스 참관기, 노키아 몰락 이후 스타트업 붐이 인 핀란드 IT 배경과 Aalto 대학 워크숍 프로그램
- [2019 UX London 컨퍼런스](concepts/2019-UX-London-컨퍼런스.md) — Katie Koch(Spotify)의 서비스 디자인 협업 사례(프리미엄 듀오·인도네시아 현금 결제)와 Eva-Lotta Lamm의 펜과 종이 시각화 프레임워크 '맵핑 시스템'
- [UX Conference London 2019](concepts/UX-Conference-London-2019.md) — 2019 UX London 컨퍼런스와는 다른 별개 행사, Martyn Reding의 UX-마케팅 역사적 대응관계, Jessica Lovegood의 UX 기반 비즈니스 성장 3원칙, Anat Treanor의 구독경제 마찰 감소, Patrick Fagan의 사이코그래픽·3중뇌 가설 기반 개인화 UX와 Angela Arnold의 마케팅-UX 협업 공간 설계 4원칙
- [2019 웹 트렌드 컨퍼런스](concepts/2019-웹-트렌드-컨퍼런스.md) — 더즈인터랙티브의 한화 금융 브랜드 'LifePlus' 웹사이트 기획 사례로 본 7가지 웹 기획 전략(Life as a Service·Bold Typography·Broken-Grid Layout 등)
- [UX 심포지움 2010 세미나 리뷰](concepts/UX-심포지움-2010-세미나-리뷰.md) — 2010년 도널드 노먼·빌 벅스턴 방한 강연을 다룬 국내 UX 블로거 8인의 후기를 모은 pxd story 큐레이션, asteray의 "Definition·Theme·Insight" 3축 정리
- [UXCamp Seoul 2010 세미나 리뷰](concepts/UXCamp-Seoul-2010-세미나-리뷰.md) — 2010년 서울대에서 열린 UXcampSeoul 바캠프 후기, pxd 한상택의 "Problem Driven Goal Directed Design, 아이폰 웹어플 리디자인" 발표
- [UXCamp Seoul 2011 세미나 리뷰](concepts/UXCamp-Seoul-2011-세미나-리뷰.md) — 2011년 성균관대 UX 커뮤니티 컨퍼런스 후기: 스케치 UX, Space UX와 'Boss eXperience', 장애인 사용자 경험, 사용성 테스트 실무 팁, 아이패드·갤럭시탭·종이책 비교 연구
- [2011 pxd UI Workshop](concepts/2011-pxd-UI-Workshop.md) — 앨런 쿠퍼 『About Face』 사내 Book 스터디를 검증한 6주 실습형 퍼소나 워크숍, User Interview→Persona→Scenario→Framework Sketch 단계별 진행과 "핵심적 행동 패턴 하나"를 찾는 insight 원칙
- [UXCamp Seoul 2012 세미나 리뷰](concepts/UXCamp-Seoul-2012-세미나-리뷰.md) — 2012년 KGIT에서 열린 UXCamp Seoul 4th 후기: 마술과 UX 접목(노병욱), 수평적 사고 기반 목표 설정(정기원), Auditory UI 4요소·촉각/청각 피드백 연구, 반응카드법 기반 감성 평가
- [UXCamp Seoul 2014 세미나 리뷰](concepts/UXCamp-Seoul-2014-세미나-리뷰.md) — 2014년 서울크리에이티브랩 'behind the curtain' 주제로 열린 UXCamp Seoul 5th 후기: 레고 매뉴얼 디테일과 UX 글쓰기 팁(우상훈), 커뮤니티 디자인, Assistor PS 도구, 모바일 브로셔 시장
- [UX intensive 2011 세미나 리뷰](concepts/UX-intensive-2011-세미나-리뷰.md) — 2011년 포스코 P&S 타워 UX 세미나 후기: 터치 인터페이스 기술동향, overriding UX factor 전략론, 삼성 갤럭시/스마트냉장고 UX, 네이버 검색/모바일 UX(Family UI)
- [UX intensive 2012 암스텔담 세미나 리뷰](concepts/UX-intensive-2012-암스텔담-세미나-리뷰.md) — Adaptive Path 주최 암스텔담 UX intensive Day4(Service Design) 리뷰: 스킷드라마(즉흥연기) 프로토타이핑, 서비스 저니맵, Blueprints·Business Model Canvas
- [UX Masterclass 2019 컨퍼런스](concepts/UX-Masterclass-2019-컨퍼런스.md) — 남아공 요하네스버그 UXMC 2019 "Humanizing Innovation": Gavin Lew의 AI UX 프레임워크(Context·Interaction·Trust), Simon Herd의 어린이 헬스케어 앱 Xploro, 아프리카 leapfrog effect
- [UX Masterclass 2020 컨퍼런스](concepts/UX-Masterclass-2020-컨퍼런스.md) — 인도 PeepalDesign 주관 온라인 UX 컨퍼런스, 인디 영의 "사고 방법(thinking style)" 프레임, Facebook Desirability Toolkit, Gavin Lew의 Humanizing AI(맥락·인터랙션·신뢰), UX Alliance Design Maturity Model, 류정우(pxd)의 Data-driven User Modeling
- [UX STRAT 2020 컨퍼런스](concepts/UX-STRAT-2020-컨퍼런스.md) — pxd 후원 첫 아시아 온라인 컨퍼런스 "East + West = Strategic Design", 인스타그램 이노베이션 랩의 5단계 혁신 프레임워크(Start where you are~Evolve with your community)
- [UX World 2013 컨퍼런스](concepts/UX-World-2013-컨퍼런스.md) — 2013년 제1회 국내 UX 컨퍼런스: 이건표(UCD→참여형 디자인 전환)·제시 제임스 가렛(Elements of UX 5요소)·구글 UX 5원칙·네이버 UX 9원칙 등 국내외 UX 리더 9인의 세션 종합
- [UX World 2014 컨퍼런스](concepts/UX-World-2014-컨퍼런스.md) — 2014년 UX World Fall: LG전자 최진해의 모바일 UX 혁신 4가지(Knock on·Knock code·Smart notice·스마트 키패드), 구글 Alex Cook의 Material Design 개발 배경, 신세계 김연수의 SSG.com 통합 사례와 커머스 코어밸류 우선순위
- [UX World 2017 컨퍼런스](concepts/UX-World-2017-컨퍼런스.md) — 2017년 "UX for Digital Transformation": 구글 제러드 짐머맨의 팀 빌딩(소속감·믿음, 명성·돈·영향력), 페이스북 VR 콘텐츠 디자인 3요소(실재감·편안함·입력방식), 우버 ATG의 자율주행 UX와 지도 디자인 3원칙
- [UX World 2018 spring 컨퍼런스](concepts/UX-World-2018-spring-컨퍼런스.md) — 2018년 "Creating Innovative Customer Experiences": 카카오미니 AI 스피커의 음성 구조·대화 설계·AI 퍼소나, Waymo 자율주행 5단계 레벨과 탑승자용 360도 주행 시각화
- [UX World 2018 fall 컨퍼런스](concepts/UX-World-2018-fall-컨퍼런스.md) — 2018년 하반기 IDG 주최 컨퍼런스: Airbnb의 가치 중심 디자인과 OpenHomes 사례, 페이스북 Product Metrics 개발 절차, Frog·British Telecom 차세대 TV UX 재설계
- [UI17 컨퍼런스와 회의 퍼실리테이션 기법](concepts/UI17-컨퍼런스와-회의-퍼실리테이션-기법.md) — 2012년 보스턴 UIE 주최 UI17 컨퍼런스 워크숍 후기: 퍼실리테이터·레코더·그룹 멤버·리더 4대 롤과 페차쿠차(PECHA KUCHA) 등 10개 회의 퍼실리테이션 기법
- [CES 2013 컨퍼런스](concepts/CES-2013-컨퍼런스.md) — pxd 해외 교육 프로그램으로 참관한 2013 라스베가스 CES, LG·삼성 OLED/UHD TV 경쟁과 스마트홈 허브, 인텔 동작 인식, 토요타·아우디 무인자동차, 디지털 헬스 트렌드
- [Cooper Visual Interface Design 워크숍](concepts/Cooper-Visual-Interface-Design-워크숍.md) — 2014년 샌프란시스코 Cooper사 워크숍 후기: Research→Translate→Archetype→Document 4단계 Visual Interface Design Process와 Goal-directed Design의 근거·비전 확보 방식
- [DTALKS 샌프란시스코 디자인 세미나](concepts/DTALKS-샌프란시스코-디자인-세미나.md) — 2015년 D.Camp 세미나: 우버식 목적 최적화 화면·Material Design 리소스 확산으로 본 샌프란시스코 디자인 트렌드, 한국-미국 디자이너 협업 문화 비교
- [헤럴드 디자인 포럼 컨퍼런스](concepts/헤럴드-디자인-포럼-컨퍼런스.md) — 2012년 헤럴드 디자인 포럼 Day 1 후기: 안도 타다오·하라 켄야·오준식·장 샤오강 강연으로 본 아시아 디자인 정체성과 디자인의 사회적 책임
- [Spectrum Con 2019 컨퍼런스](concepts/Spectrum-Con-2019-컨퍼런스.md) — 원티드×디자인 스펙트럼 공동 주최 컨퍼런스, 라인(이정영)의 디자이너 역할 확장과 조직 대응, 쿠팡(조나단 정)의 비즈니스 드리븐 디자인 사례
- [SAP Executive Summit 2019 컨퍼런스](concepts/SAP-Executive-Summit-2019-컨퍼런스.md) — '혁신과 클라우드' 주제의 SAP 행사 후기, 두산중공업·SK하이닉스의 디자인 씽킹·애자일 적용과 제조업 서비스화·플랫폼 비즈니스 전환 사례
- [Tech Planet 2012 컨퍼런스](concepts/Tech-Planet-2012-컨퍼런스.md) — SK Planet 주최 컨퍼런스 후기: Adaptive Path 재민 헤게만의 UX→SX(Service Experience) 전환론, 이태일 교수의 Natural UI·제스처 연구, Sunnyloft의 프라이빗 SNS 'Episode' 기획 사례
- [Wanted con 2020 컨퍼런스](concepts/Wanted-con-2020-컨퍼런스.md) — 원티드 주최 디자인 컨퍼런스, 쿠팡 조나단 정의 "The power of Unlearning": Simplicity·Consistency·Innovation 세 가지 관점을 데이터로 relearning하는 사례
- [UX와 UI의 차이](concepts/UX와-UI의-차이.md) — UI는 사람과 정보기기가 접하는 면, UX는 사용자가 제품·서비스를 통해 얻는 총체적 경험
- [검색 UX 설계](concepts/검색-UX-설계.md) — 통합검색의 컬렉션 나열 대신 소수 소스를 예측 가능한 순서로 배치하는 검색 파레토 법칙, Search User Goals 3분류, 검색어 자동완성 UX
- [게임 기반 재난 안전 교육 디자인](concepts/게임-기반-재난-안전-교육-디자인.md) — 세월호 참사 이후 pxd-놀공발전소가 기획한 초·중학생 대상 재난 안전 보드게임 '어루만지다', 몰입(용도·추상화·메시지 중심)과 주체적 경험을 핵심으로 한 '안전한 우리 도시' 설계형 게임 컨셉
- [게임 디자인 사례 — 추억은 방울방울](concepts/게임-디자인-사례-추억은-방울방울.md) — 학창시절 책상 놀이(동전따먹기·볼펜따먹기·지우개따먹기)를 아이패드 게임으로 재현하려 한 컨셉 스케치, 아날로그 놀이의 물리적 상호작용을 멀티터치 제스처로 치환
- [게임 디자인 워크숍 기법](concepts/게임-디자인-워크숍-기법.md) — 놀공발전소와의 협업 워크숍으로 본 팅커링(Tinkering)·코어 메카닉·베이스라인 시프팅 신드롬 등 게임 디자인 방법론
- [게임 산업 구조와 Uxer 역할](concepts/게임-산업-구조와-Uxer-역할.md) — 국내 게임 시장의 "익숙함을 팔다" 전략과 게임=국가 비유, Uxer의 5대 책임 영역(user researcher·게임/서비스 디자이너·UX 디자이너·UI 디자이너·UI 아티스트)
- [게임 신규유저 적응 격차](concepts/게임-신규유저-적응-격차.md) — 온라인 게임 '프리스타일' 사례로 본 신규-기존 유저 실력 격차의 원인: 회원등급제 실효성, 비매너 유저 대응, 비속어 필터링의 부작용, 밸런싱 장치 부재
- [게임 유저 데이터 모델링](concepts/게임-유저-데이터-모델링.md) — MMORPG 행동 로그 기반 행위기반 클러스터링·시퀀스 마이닝·네트워크 분석으로 사용자 유형을 입체적으로 이해하는 방법론
- [매직 서클과 LXD](concepts/매직-서클과-LXD.md) — 놀공발전소 피터공의 게임 정의 Rules+Player=Play, 규칙이 안 보일수록 좋은 게임이라는 매직 서클(Magic Circle), 다수의 패자를 배려하는 LXD(Loser Experience Design) 개념
- [그룹형 SNS UX 설계](concepts/그룹형-SNS-UX-설계.md) — 2012년 NHN 밴드 UX 리뷰로 본 그룹 SNS 기획의 4대 기본기: 친구 유입·속도/안정성·적시 알림·기능 접근성
- [네비게이션 레이블링](concepts/네비게이션-레이블링.md) — 메뉴·카테고리·링크 텍스트를 설계하는 원칙: 사용자 언어·설명성·상호 배타성·일관성 등 6가지 레이블 품질 기준
- [대체현실게임(ARG)](concepts/대체현실게임-ARG.md) — 제인 맥고니걸 『누구나 게임을 한다』와 놀공발전소 『노력금지』로 본 현실을 플레이 공간으로 삼는 반도피주의 게임, 목표·규칙·피드백·자발적 참여 4가지 본질과 퀘스트 투 런 교육 모델
- [데이터 기반 UX (DDUX)](concepts/데이터-기반-UX-DDUX.md) — DDUX 마인드셋 3가지(의사결정자·개입·증거)와 디자인 씽킹 강화·초개인화를 연결하는 데이터 기반 UX 방법론
- [데이터 기반 퍼소나](concepts/데이터-기반-퍼소나.md) — GA4 로그와 텍스트 임베딩·클러스터링으로 유입 검색어 기반 사용자 유형을 세분화하는 방법론
- [래피드 퍼소나 워크숍](concepts/래피드-퍼소나-워크숍.md) — 우선순위 도출과 Key flow 검증을 위해 반나절 만에 만드는 간이 퍼소나, 2013년 한국 인지과학 학술대회 User Modeling 워크샵 사례
- [디자인 사고](concepts/디자인-사고.md) — IDEO의 팀 브라운이 체계화한 혁신 방법론
- [디자인 스프린트](concepts/디자인-스프린트.md) — 구글의 협업 기반 문제해결 프레임워크: Understand-Define-Sketch-Decide-Prototype-Validate 6단계 프로세스와 원격 진행 시 유의점
- [마인드스토밍 퍼실리테이션 기법](concepts/마인드스토밍-퍼실리테이션-기법.md) — 외부 퍼실리테이터 이유진의 2012 pxd talks 09 워크숍, Mind Opening→Storming→Sharing→Evaluation 4단계로 참여 분위기를 조성하는 기법, 워크숍 아이디어가 한강 월간회의·pxd 장터로 실현된 사례
- [아이디에이션과 수평적 사고](concepts/아이디에이션과-수평적-사고.md) — 에드워드 드 보노의 수평적 사고와 무작위 단어·이미지 연상 기법을 활용한 창의적 아이디어 발상법
- [디자인 의사결정 프레임워크](concepts/디자인-의사결정-프레임워크.md) — 목표(Goals)·인풋(Inputs)·계획(Plan) 3단계로 창조적 과정과 논리적 추론을 결합하는 팀 기반 의사결정 모델
- [디자인 정보분류체계](concepts/디자인-정보분류체계.md) — 2011년 한국디자인학회 논문 기반 컨퍼런스 발표, 제품·시각·서비스·UX 등에 흩어진 디자인 지식을 다차원 구조형 분류법으로 통합하는 프레임워크
- [디자인 컨설팅 프로세스](concepts/디자인-컨설팅-프로세스.md) — 더블 다이아몬드 기반 문제 정의-해결 구조, 문제와 현상의 구분, Goal-Strategy-Tactics 3레이어 전달법을 포함한 UX 컨설팅 실무 방법론
- [디자인 통찰과 관점 전환](concepts/디자인-통찰과-관점-전환.md) — 통찰(insight)은 훈련 가능한 기술: 공감·문제 구조화·어피니티 다이어그램·발상기법을 통한 관점 전환 방법론
- [디지털 전환기 UX 설계 원칙](concepts/디지털-전환기-UX-설계-원칙.md) — 화면 터치포인트를 넘어 물리적 공간(아마존 고)까지 아우르는 맥락 인지·멀티모달 인터랙션, Prediction→Understanding·Reactive→Proactive 전환으로 본 초개인화 UX 설계 원칙
- [엔터프라이즈 UX 설계 원칙](concepts/엔터프라이즈-UX-설계-원칙.md) — 업무 관점으로 화면을 재설계하는 엔터프라이즈 UX, 기존/신규 유저×기능 매트릭스와 사용자 멘탈모델 파악을 통한 업무 퍼포먼스 갭 축소
- [추상화와 구체화](concepts/추상화와-구체화.md) — 공통 속성을 추출해 단순화하는 추상화와 지각 가능한 디테일을 채우는 구체화, 지도의 역설과 잘못된 추상화(Wrong Abstraction) 함정
- [버스 정보 안내판 정보 설계](concepts/버스-정보-안내판-정보-설계.md) — 서울 BIT 번호 정렬 규칙과 정류장 맥락을 고려한 버스 노선도 리디자인 사례
- [브랜드 DNA와 경험 원칙 카드](concepts/브랜드-DNA와-경험-원칙-카드.md) — Euro IA 2013 크리스토퍼 리 볼의 버진 애틀랜틱 웹사이트 리디자인 사례, 브랜드 DNA를 경험 원칙 카드로 번역해 오프라인·온라인 경험 단절을 줄이는 방법
- [블로그 검색 노출과 크롤링 정책](concepts/블로그-검색-노출과-크롤링-정책.md) — 네이버 블로그 검색의 opt-in과 일반 검색엔진의 opt-out 크롤링 정책 차이, 최신 글 가중치가 검색 노출에 미치는 영향
- [비평적 디자인과 추측적 디자인](concepts/비평적-디자인과-추측적-디자인.md) — SoftDevice의 관찰→아이디에이션→스케칭→실행 개발 주기와 비평적 재조정 개념, Hospital Radio·Toyota Fine-Comfort Ride 사례
- [빅게임 디자인](concepts/빅게임-디자인.md) — Frank Lantz의 빅게임(Big Game) 정의와 pxd '사도세자의 비밀' 워크숍 기획 사례, 코어 메카닉·상소 찬스 등 게임화 장치
- [사물 관찰과 제품 디자인 디테일](concepts/사물-관찰과-제품-디자인-디테일.md) — 익숙한 사물을 낯설게 관찰하는 훈련과 제조·구조적 제약 파악을 통한 제품 디자인 디테일 이해, 오프라인 사물 경험이 메타포·스큐어모피즘으로 디지털에 계승되는 원리
- [사용성 테스트 랩 구성](concepts/사용성-테스트-랩-구성.md) — Test Room·Observation Room으로 구성된 UT Lab 물리적 구조, 전 세계 31개 랩 사례와 원격 리서치·현장 조사로 확산되는 추세
- [사용자 여정 분석](concepts/사용자-여정-분석.md) — 사용자가 목적 달성을 위해 거치는 단계와 경험을 파악하는 방법론
- [사용자 연구 사례 — mMessenger](concepts/사용자-연구-사례-mMessenger.md) — pxd·SK텔레콤 mMessenger 개발 사용자 연구 사례, 저널링+컨텍스추얼 인쿼리로 실시간 채팅 대신 SMS를 선택하는 이유를 규명, 2010년 ACM interactions 지 소개
- [사용자 연구 사례 — 스마트폰 정의 인식조사](concepts/사용자-연구-사례-스마트폰-정의-인식조사.md) — 2009년 런던 사용자 대상 "피처폰과 스마트폰의 차이" 정성 조사, OS별로 갈리는 스마트폰 멘탈 모델과 첫 응답만으로 만족도를 단정하면 안 된다는 리서치 시사점
- [사용자 행동 패턴](concepts/사용자-행동-패턴.md) — 반복되는 행동의 규칙성을 발견하고 맥락을 이해하는 UX 리서치의 핵심 개념
- [10대의 영상 중심 커뮤니케이션 행동](concepts/10대와-영상-중심-커뮤니케이션.md) — ALIVE 서비스의 타겟 예측 실패 사례로 본 미국 10대(Gen Z)의 영상 중심 학습·소통 행동, 밀레니얼과 대비되는 영상에 대한 태도 차이
- [센스메이킹과 인문학적 비즈니스 리서치](concepts/센스메이킹과-인문학적-비즈니스-리서치.md) — 크리스티안 마두스베르그·미켈 라스무센의 『우리는 무엇을 하는 회사인가?』: 디폴트 사고 vs 센스메이킹 대비, 현상학·민족지학·귀추법 세 방법론적 뿌리, 스포츠 용품·콜로플라스트 사례로 본 명료함의 순간
- [수렴적 디자인 프로세스](concepts/수렴적-디자인-프로세스.md) — 쿠퍼(Cooper)식 목표지향적 디자인 6단계에서 인터랙션·시각·산업 디자이너를 병렬 협업시켜 "디자인 언어 원형"으로 수렴시키는 방법론
- [Workflow 협업 설계 프로세스](concepts/Workflow-협업-설계-프로세스.md) — pxd 초창기(2010) 3인 UI 디자이너의 Day1~Day3 협업 절차: 공통 목표→공통 양식→개별 실행 순으로 하나의 Workflow 기획서를 함께 그리는 방법론
- [스마트폰 스피커홀 위치와 통화 사용성](concepts/스마트폰-스피커홀-위치와-통화-사용성.md) — 갤럭시 S3 스피커홀 정렬 논란과 "form follows function or not" 질문, 12명 대상 rapid UT로 밝힌 귓바퀴 연골 통증·소음차단-명료도 요구 충돌
- [아이트래킹](concepts/아이트래킹.md) — 시선 추적 기술로 사용자의 시각적 주의 패턴을 분석하는 UX 리서치 방법론
- [운전 중 스마트폰 사용 행태](concepts/운전-중-스마트폰-사용-행태.md) — 서울대 UX 연구실의 Machine Log Data + Behavior Data 기반 Deep Data Analysis, 운전 중 스마트폰 사용 5가지 세션 분류(Cycle Thru·Come up with·Active Pursuing·Conversing·Defer)
- [원격 사용자 조사](concepts/원격-사용자-조사.md) — 해외 등 물리적으로 먼 사용자를 모더레이팅 유무에 따라 원격으로 조사하는 방법론과 도구 선택 기준
- [웹 아카이브 리서치 도구](concepts/웹-아카이브-리서치-도구.md) — oldapps.com·Wayback Machine으로 애플리케이션·웹사이트의 과거 버전을 추적해 변화 추이로 트렌드를 가늠하는 리서치 도구
- [윷놀이 UX와 뒷도 규칙 모호성](concepts/윷놀이-UX와-뒷도-규칙-모호성.md) — 지역마다 다른 '뒷도' 규칙 논쟁으로 본 인터랙티브 시스템의 에지 케이스 설계 원칙
- [의료 서비스 디자인](concepts/의료-서비스-디자인.md) — 메이요 클리닉 사례로 보는 환자 중심 서비스 철학과 경험 신호(기능적·기계적·인간적) 설계
- [익스트림 프로토타이핑과 즉흥연기](concepts/익스트림-프로토타이핑과-즉흥연기.md) — 김창준의 애자일 프로토타이핑 워크숍: 짧은 계획과 즉각 실행·피드백을 반복하는 문제해결 접근, 즉흥연기 CROW 기법과 Prospective Hindsight
- [인터넷 검색 기반 리서치 기법](concepts/인터넷-검색-기반-리서치-기법.md) — 프로젝트 초반 짧은 시간 안에 인터넷 검색으로 필요한 정보를 확보하는 민첩한 리서치(Rapid Research) 접근법
- [인포그래픽 유형 분류](concepts/인포그래픽-유형-분류.md) — 디자인 기획 과정에서 활용되는 인포그래픽 5대 유형: 흐름·관계·구조·모델링·정량 데이터
- [정보 구조 설계 IA](concepts/정보-구조-설계-IA.md) — 서비스의 전체 구조를 지탱하는 뼈대로서의 정보 구조
- [정보 디자인 원칙](concepts/정보-디자인-원칙.md) — 사용자가 정보를 효과적으로 인지하고 행동할 수 있도록 설계하는 원칙
- [정보 시각화](concepts/정보-시각화.md) — 2012년 pxd talks 첫 세션 강규영 강연: 데이터 기반 의사결정 시대에 디자이너가 갖춰야 할 정보 시각화 역량과 "시각화를 왜 하는가"라는 근본 질문
- [일본 제품 비교 차트 디자인](concepts/일본-제품-비교-차트-디자인.md) — 소니·au·docomo 제품 사양 비교표 분석으로 본 일본식 비교 차트의 4가지 정보 설계 패턴: 고밀도 구성, 각주+페이지 참조, 해당/미해당 시각 구분, 제품 이미지 활용
- [여행 콘텐츠 정보 설계](concepts/여행-콘텐츠-정보-설계.md) — 캐릭터 서사·일러스트레이션 기반으로 복잡한 여행 정보를 초심자 친화적으로 구조화한 가이드북 작업 사례
- [퓨처 리딩과 디지털 독서 경험](concepts/퓨처-리딩과-디지털-독서-경험.md) — Euro IA 2013 "Future Reading" 세션: 페이지·챕터 경계가 사라진 디지털 콘텐츠에서 콘텐츠 성격에 맞는 탐색 도구와 다차원 내비게이션을 설계해야 한다는 제언
- [컨텍스추얼 인쿼리](concepts/컨텍스추얼-인쿼리.md) — 사용자의 현장에서 진행하는 에스노그래피형 필드 리서치, Beyer & Holtzblatt의 4대 원칙
- [코크리에이션 워크숍 설계와 퍼실리테이션](concepts/코크리에이션-워크숍-설계와-퍼실리테이션.md) — Euro IA 2013 쾬 페터스 발표: 참가자·모디레이터·공간 3요소와 프레이밍·로터스 블라섬·시리어스 플레이 등 코크리에이션 워크숍 설계·진행 기법
- [콘텐츠 전략과 메시지 프레임워크](concepts/콘텐츠-전략과-메시지-프레임워크.md) — 크리스티나 할버슨의 Substance·Structure·Workflow·Governance 4대 영역과 메인/부수적 메시지로 콘텐츠를 사전 기획하는 방법론
- [QR코드 UX 설계](concepts/QR코드-UX-설계.md) — 개별 콘텐츠마다 QR코드를 다는 대신 '멀티미디어 인덱스 페이지' 하나로 연결해 포고스티킹을 줄이는 오프라인-온라인 연결 설계 원칙
- [키스크린과 인터랙션 프레임웍](concepts/키스크린과-인터랙션-프레임웍.md) — 사용자·사업·기술 세 축을 종합한 UI 프로젝트의 전환점
- [상태 전이 모델링](concepts/상태-전이-모델링.md) — 상태·사건·액션·전이로 시스템을 분해해 기획의 빈틈을 드러내는 도구
- [디자인 경영과 혁신 사례](concepts/디자인-경영과-혁신-사례.md) — 레고의 위기와 D4B(Design for Business) 모델로 본 창의성·전략 통합
- [User Story와 애자일](concepts/User-Story와-애자일.md) — 엉성하게 시작해 Iteration으로 정확성을 높이는 애자일 사고와 User Story 활용법
- [보험 서비스 UX](concepts/보험-서비스-UX.md) — 시니어 고객-설계사-콜센터 삼각 관계 분석과 다채널 리서치 사례
- [도그푸딩](concepts/도그푸딩.md) — 만드는 사람이 쓰는 사람의 시점을 직접 경험하는 6단계 프로세스
- [린 스타트업과 피벗](concepts/린-스타트업과-피벗.md) — 에릭 리스의 10가지 피벗 종류와 자기 강점 기반의 방향 전환
- [Lean UX와 Agile UX](concepts/Lean-UX와-Agile-UX.md) — 디자인 씽킹·애자일·린 스타트업을 통합한 협업 중심 UX 방법론과 실무 적용 교훈
- [MVP 중심 애자일 프로젝트 운영](concepts/MVP-중심-애자일-프로젝트-운영.md) — 디지털 전환 프로젝트 1년 리드 경험: 사용자 여정·멘탈모델 이해 후 MVP 우선 릴리즈, 디자인이 시작점이 되는 2주 스프린트 구조, 지라 기반 투명한 협업
- [멘탈 모델 리서치 방법](concepts/멘탈-모델-리서치-방법.md) — 인디 영의 Mental Models 방법론: 사전 연관성 도표 작성을 통해 사용자 조사 범위와 리크루팅 계획을 체계적으로 수립하는 3단계 프로세스
- [모바일 앱 UX 리뷰](concepts/모바일-앱-UX-리뷰.md) — 실제 이용 패턴을 기반으로 카카오지하철 앱을 분석한 사용자 관점 UX 리뷰 방법
- [미디어 콘텐츠에서 UX 인사이트 발견하기](concepts/미디어-콘텐츠에서-UX-인사이트-발견하기.md) — 일본 드라마 '중쇄를 찍자!'로 본 편집자-작가-독자와 PM-디자이너-사용자 구조의 유사성, VOC·A/B 테스트·프로토타이핑 관찰
- [사용자 인터뷰 기법](concepts/사용자-인터뷰-기법.md) — 효과적인 인터뷰를 위한 10가지 가이드라인, 다이어리 스터디, 전문가 인터뷰 기법(pre-mortem·프레임 깨기)
- [합성 사용자 Synthetic User](concepts/합성-사용자-Synthetic-User.md) — AI 퍼소나·합성 퍼소나를 근거(Anchor)·정합(Align)·행동(Act) 세 축으로 평가하는 AAA 프레임워크, ReOpsAI·Synthetic Users·Simile 등 사례 비교
- [해외 사용자 조사 프로세스](concepts/해외-사용자-조사-프로세스.md) — 현지 업체 선정, 리쿠르팅, 동시통역, 랩업 등 출장형 해외 사용자 조사의 5단계 프로세스와 실무 용어(스크리너/치터/no-show)
- [해외 클라이언트 UX 프로젝트 협업](concepts/해외-클라이언트-UX-프로젝트-협업.md) — 신기술 검증을 위해 해외 클라이언트와 진행한 Discover-Define-Develop-Validate-Deliver UX 프로세스와 Wizard of Oz 프로토타이핑, 클라이언트 소통 노하우
- [해외 UX 컨퍼런스 큐레이션](concepts/해외-UX-컨퍼런스-큐레이션.md) — 2018/19년 참가할 만한 해외 UX 컨퍼런스 8곳(UI/UX Conference China·EURO IA·Service Design Global Conference·Interaction Conference 등) 큐레이션, 참가 3대 효용(업계 흐름·네트워킹·자기 점검)
- [현장 관찰 리서치 방법론](concepts/현장-관찰-리서치-방법론.md) — 얀 칩체이스의 창발적 행위 포착·극단적 사용자·직접 체험·한계치 맵 등 현장 기반 리서치 원칙
- [프로토타이핑 툴 비교](concepts/프로토타이핑-툴-비교.md) — 시나리오 검증 vs 인터랙션 확인 목적에 따른 Flinto·InVision·Proto.io·Origami·Framer 툴 선택 가이드
- [위자드 오브 오즈 프로토타이핑](concepts/위자드-오브-오즈-프로토타이핑.md) — 실제로 작동하지 않는 기술을 사람이 뒤에서 조작해 진짜처럼 느끼게 하는 WOZ 기법, AR 글라스 사례로 본 Discover/Define/Deliver 단계별 적용
- [UX 성숙도 모델](concepts/UX-성숙도-모델.md) — NN/g 8단계·InVision 5단계 등 조직의 UX 현재 위치 진단
- [동기강화 상담과 조직관리](concepts/동기강화-상담과-조직관리.md) — MI 기법을 조직관리와 PM 코칭에 적용하는 방법
- [제조업 서비스화와 UX](concepts/제조업-서비스화와-UX.md) — TAM 기술 수용 모형과 지각된 유용성·사용 용이성
- [짝 디자인](concepts/짝-디자인.md) — 짝 프로그래밍에서 착안한 두 디자이너의 협업 방식, 제너레이터·신테사이저 역할 분담과 디자인 얼라우드·크로스 폴리네이션
- [창조적 축적과 마인크래프트 세대](concepts/창조적-축적과-마인크래프트-세대.md) — 『축적의 시간』의 창조적 축적 부재 진단, 모방추격형 산업 발전 모델 vs 창조적 축적 지향 체제, 마인크래프트 세대와 사티아 나델라 발언, 디자인 씽킹·프로슈머 흐름과의 연결
- [챗봇 UX 설계](concepts/챗봇-UX-설계.md) — 챗봇 3가지 가치와 설계 원칙, CUI→VUI 전환의 중간 단계
- [좋은 디지털 경험의 조건](concepts/좋은-디지털-경험의-조건.md) — 목적 달성 효율화·의미 있는 추천·유대감 형성·감성적 만족, 좋은 디지털 서비스의 4가지 공통 속성
- [글쓰기 기술](concepts/글쓰기-기술.md) — 3S 원칙(Short·Story·Seat)·뇌를 움직이는 7원칙·독서+실천 기반의 글쓰기 방법론
- [피라미드 원칙과 논리적 글쓰기](concepts/피라미드-원칙과-논리적-글쓰기.md) — 바바라 민토 『논리의 기술』의 피라미드 원칙: 수직적·수평적 관계, 연역·귀납 추론, MECE 원칙 기반 논리적 글쓰기 구조화 기법
- [에디터 직업론과 콘텐츠 큐레이션](concepts/에디터-직업론과-콘텐츠-큐레이션.md) — 좋아하는 것으로부터 좋은 것을 골라내는 에디터의 본질, 3I 원칙(Inform·Inspire·Entertain)과 팀워크·소명의식
- [온라인 리치텍스트 에디터](concepts/온라인-리치텍스트-에디터.md) — Iframe·Markdown·Inline·Medium 4가지 온라인 에디터 유형과 글쓰기 집중·스타일 통일·WYSIWYG 완성도라는 변화의 세 동인
- [온라인 주문 프로세스 UX](concepts/온라인-주문-프로세스-UX.md) — 피자 주문 과정을 순서도로 분석해 메뉴 미결정 상태의 과도한 옵션이 결정을 방해하는 지점을 짚은 2010년 초기 UX 사례
- [에버노트 사진찍기 기능](concepts/에버노트-사진찍기-기능.md) — 사진·문서·POST-IT 3대 모드로 아날로그 메모·화이트보드를 매체별 스캔 알고리즘으로 디지털화하는 에버노트 기능
- [적정기술과 사회적 디자인](concepts/적정기술과-사회적-디자인.md) — 스타일링이 아닌 서비스·시스템으로서의 디자인이 적정기술을 '적정'하게 완성한다: 인간 중심 맥락 이해와 다학제적 통합조정자로서의 디자이너
- [레고 활용 UX 워크숍 기법](concepts/레고-활용-UX-워크숍-기법.md) — 아이디어 실체화·공동 스토리텔링·놀이화를 돕는 레고, LEGO Serious Play 4단계 프로세스와 pxd 신입사원 팀빌딩 적용 사례
- [화면 설계서 작성법](concepts/화면-설계서-작성법.md) — 표지·히스토리·목차·인터랙션 가이드·IA·키스크린·플로우 7가지 성분과 Lean/Agile vs UCD/Waterfall 방법론 선택
- [결정적 순간 (Moment of Truth)](concepts/결정적-순간-Moment-of-Truth.md) — 스칸디나비아 항공 얀 칼슨·노드스트롬·구글 마이크로모먼츠·P&G 사례로 본, 고객 여정에서 서비스 성패를 가르는 결정적 순간을 찾는 방법론
- [인스타그램 디자인 프로세스](concepts/인스타그램-디자인-프로세스.md) — 넷플릭스 다큐 앱스트랙트 '이안 스폴터' 편으로 본 인스타그램 디자인팀의 직관적 네이밍·과감한 안 병행·데이터+개별 관찰 균형 프로세스
- [사용성 비교 평가](concepts/사용성-비교-평가.md) — 경쟁사 대비 자사 제품의 사용성을 측정하는 경쟁력 평가 방법론, 전문가 평가와 사용자 평가의 병행과 승패보다 전략 도출에 집중하는 원칙
- [Task 기반 사용성 평가](concepts/Task-기반-사용성-평가.md) — 정지된 시안 대신 프로토타입으로 실제 Task를 수행하게 해 사용성을 검증하는 방법, 프레임웍 단계에서의 조기 도입 효과와 한계
- [시간 위의 디자인](concepts/시간-위의-디자인.md) — 공간(화면) 중심 UI 설계에서 이벤트·시점 단위로 상호작용을 설계하는 시간 중심 설계로의 전환, "기능을 공간이 아닌 시간에 그려야 한다"
- [시장 조사와 사용자 조사의 차이](concepts/시장-조사와-사용자-조사의-차이.md) — ESOMAR로 본 시장 조사와 사용자 조사의 지향점 차이, P&G 래플리 회장의 Two Moments of Truth가 P&G를 디자인 중심 조사로 이동시킨 사례
- [Next Billion Users와 비문자 중심 UX](concepts/Next-Billion-Users와-비문자-중심-UX.md) — 저문해율 인구를 위한 이미지·음성·영상 중심 UX 3원칙, 인도의 스마트폰 첫 사용자 사례와 한국 노안 사용자·티몬 라이브 커머스 사례

## 디자인 일반
- [디자인 배기지](concepts/디자인-배기지.md) — 디자인된 대상에 담긴 보이지 않는 정치적·사회적 함의를 가리키는 개념, 라바콘·빅토리아 시대 의자·라디오 안락의자 사례로 본 디자인과 커뮤니케이션의 관계
- [멀티터치 바르셀로나와 인터페이스 예술](concepts/멀티터치-바르셀로나와-인터페이스-예술.md) — 스페인 실험적 디자인 그룹 Multitouch Barcelona의 "Real human interface" 영상 퍼포먼스로 본, 실용적 UX가 아닌 철학적 질문으로서의 인터페이스 예술
- [베를린 비엔날레](concepts/베를린-비엔날레.md) — 2018년 제10회 베를린 비엔날레(BBX) 관람기, 슬로건 'We don't need another hero'와 자기보존(Self-Preservation)을 화두로 한 탈식민주의 큐레이션, ADK·KW·ZK/U 전시장의 신체적 체험형 설치·미디어 아트
- [스위스 바젤 아트페어](concepts/스위스-바젤-아트페어.md) — '아트 올림픽'이라 불리는 2012 아트 바젤 참관기, 실험적 작품보다 완성도 있어 보이는 작품이 더 잘 팔리는 판매 패턴 관찰과 전용 모바일 앱 기반 관람 경험
- [사이니지와 웨이파인딩 디자인](concepts/사이니지와-웨이파인딩-디자인.md) — 『Left, Right, Up, Down』 도서 리뷰로 본, 사인에 쓰인 폰트명·제작사를 투명하게 공개하는 레퍼런스의 가치와 MACBA 사인 디자인 사례
- [스팀펑크 디자인 미학](concepts/스팀펑크-디자인-미학.md) — 19세기 증기 기관 시대의 기계 미학이 SF·영화·게임·제품 디자인·GUI에 적용된 레트로퓨처리즘 양식
- [오브젝트 타이포그래피 광고 디자인](concepts/오브젝트-타이포그래피-광고-디자인.md) — 아르헨티나 에이전시 Mother Argentina·디자이너 파블로 알피에리가 모토로라 뮤직폰 Motorokr i856 광고에서 음향기기 이미지로 "MOTOROKR" 텍스트를 조립한 오브젝트 타이포그래피 사례
- [일러스트레이션 패브릭 소품 제작](concepts/일러스트레이션-패브릭-소품-제작.md) — 디지털 프린팅으로 일러스트를 패브릭에 옮겨 월행잉→캔버스 액자→파우치→인형 순으로 시행착오를 거친 사이드 프로젝트 사례
- [제품 디자인 사례 — 그림자 파라솔](concepts/제품-디자인-사례-그림자-파라솔.md) — 감온변색 안료로 나무 그늘의 일렁이는 질감을 재현하는 파라솔 컨셉, 일상 관찰에서 출발한 pxd 다이어리 아이디어 스케치
- [제품 디자인 사례 — 보이스 몽타주](concepts/제품-디자인-사례-보이스-몽타주.md) — 범죄수사의 음성 몽타주 분석 관행에서 착안, 통화·녹음 목소리를 분석해 얼굴을 자동 생성하는 pxd '소소한 이야기' 아이디어 스케치
- [제품 디자인 사례 — iMac 스탠드](concepts/제품-디자인-사례-iMac-스탠드.md) — 거북목 방지를 위한 개인적 불편에서 출발해 상용 스탠드 구매, 이후 '거울 반사' 컨셉 iMagic Stand로 아이디에이션이 확장된 pxd 다이어리 사례
- [제품 디자인 사례 — Money Printed Tissue](concepts/제품-디자인-사례-Money-Printed-Tissue.md) — 집들이 선물 화장지에 지폐 도안을 인쇄한 서류가방·사과 상자 모양 곽티슈 컨셉, 일상 관찰에서 출발한 pxd 다이어리 아이디어 스케치
- [제품 디자인 사례 — pxd 달력카드](concepts/제품-디자인-사례-pxd-달력카드.md) — 1년 전체를 한 장에 담아 지난 날도 계속 조망하게 하는 pxd GUX Design Center의 2013년 탁상 달력 카드, 음각 가이드선·목표 체크 공간·자석 받침대 거치 구조로 완성한 소량 제작 굿즈 사례
- [제품 디자인 사례 — pxd 머그컵](concepts/제품-디자인-사례-pxd-머그컵.md) — pxd 창립 10주년 기념 아이템으로 자체 머그컵을 디자인하고 공방 시제품 제작을 계획한 초기 단계 사내 다이어리 사례
- [제품 디자인 사례 — pxd 서류봉투](concepts/제품-디자인-사례-pxd-서류봉투.md) — 형압 로고·맞춤 아일렛·세로 레이아웃 등 "비용을 추가하여" 원칙으로 완성한 pxd 전용 서류봉투(Flat Bag) 자체 디자인 사례
- [제품 디자인 사례 — pxd 손목시계](concepts/제품-디자인-사례-pxd-손목시계.md) — 매 정각마다 pxd 로고 심볼이 완성됐다 흐트러지는 손목시계 컨셉스케치, Lean UX Lab이 인터랙티브 웹 프로토타입으로 구현한 사내 굿즈 아이디어 사례
- [제품 디자인 사례 — pxd 스케줄러](concepts/제품-디자인-사례-pxd-스케줄러.md) — 개인용 종이 시간표에서 출발해 외부 배포용 제품으로 재디자인한 사내 스케줄러, 탁상형 구조 특허출원과 대량생산 가능성 검증 과정
- [제품 디자인 사례 — pxd 이동식 책장](concepts/제품-디자인-사례-pxd-이동식-책장.md) — GUI 디자이너를 위한 사내 공용 도서 책장을 리스트→운영방침→컬러 레이블 관리 시스템으로 단계적으로 완성한 pxd 다이어리 사례
- [제품 디자인 사례 — pxd 일러스트레이션 달력](concepts/제품-디자인-사례-pxd-일러스트레이션-달력.md) — 개인 취미로 그린 일러스트를 엽서형 달력으로 자체 제작, SNS 공개 후 사내 확산을 거쳐 회사 지원으로 클라이언트 배포용 굿즈까지 확장된 상향식 pxd 다이어리 사례
- [제품 디자인 사례 — pxd 종이컵](concepts/제품-디자인-사례-pxd-종이컵.md) — 손님 접대용으로만 쓰는 pxd 종이컵 표면을 브랜드 카피 매체로 활용, 클라이언트와의 반복 상황에 유머로 대응하는 커뮤니케이션 도구로 만든 사내 소모품 디자인 사례
- [제품 디자인 사례 — TFD 투명 플렉서블 디스플레이](concepts/제품-디자인-사례-TFD-투명디스플레이.md) — LGD 주축 국책과제에서 pxd가 담당한 미래 디스플레이 활용 시나리오 설계, Weak Signal 기반 맥락조사와 투명 디스플레이 4대 활용 유형
- [제품 디자인 사례 — 컨셉트 음료](concepts/제품-디자인-사례-컨셉트-음료.md) — 순수 미네랄워터에 뚜껑 Cap 장치로 영양성분을 선택 첨가하는 컬러비타민워터 컨셉, 사내 공유 후 실제 특허출원으로 이어진 pxd 다이어리 사례
- [타이포그래피와 서체 디자인](concepts/타이포그래피와-서체-디자인.md) — 고바야시 아키라 세미나로 본 자간·커닝 중심 서체 설계 원칙, 글자 파생 제작 프로세스, 다국어 기업서체 디렉션 역할
- [편집 디자인과 정보 접근성](concepts/편집-디자인과-정보-접근성.md) — 가이드 캐릭터·독자 여정 기반 챕터 구성·일러스트 활용으로 낯선 정보의 진입 장벽을 낮추는 편집 설계 원칙
- [pxd 로고타입 디자인 비화](concepts/pxd-로고타입-디자인-비화.md) — 애플 로고 황금비 이미지를 계기로 GUI 디자이너가 밝힌 pxd 로고타입의 실제 작도 원리(x-height 미세 조정) 일화

## 서비스 디자인
- [고령친화 디자인과 100세 시대](concepts/고령친화-디자인과-100세-시대.md) — Age Friendly 개념과 시니어를 문제 해결의 주체로 세우는 고령친화 디자인 원칙, 2013년 서울디자인재단 100세 시대 디자인 세미나 후기
- [다크매터와 트로이의 목마](concepts/다크매터와-트로이의-목마.md) — 이승호 교수의 공공 서비스 디자인 전략 프레임워크: 표면적 의뢰(트로이의 목마) 뒤 시스템적 문제(암흑물질)를 해결하는 접근, 핀란드 Low2no·Participation Game·Design for Government 사례
- [리테일 매장 경험 디자인](concepts/리테일-매장-경험-디자인.md) — 애플 뉴욕 매장(Grand Plaza점·소호점) 방문기로 본 판매보다 체험을 앞세운 리테일 공간 설계 원리: 소재 대비, 캐주얼한 직원 배치, 상시 교육 프로그램
- [모바일 커머스와 시간 설계](concepts/모바일-커머스와-시간-설계.md) — 이재용 대표의 pxd talks 강연: E-커머스와 다른 M-커머스의 시간 즉시성·위치 접근성·개인화, 이벤트·타임·대화형 커머스 3유형과 "고객의 첫 일주일" 설계론
- [장바구니 UX 설계](concepts/장바구니-UX-설계.md) — 2011년 홈플러스·NS 농수산·이마트 식품 쇼핑몰 장바구니 비교, 가시성·정보 밀도·정보 충실도 3요소로 본 목적 구매형 이커머스 UX
- [모빌리티 서비스 유형과 MaaS](concepts/모빌리티-서비스-유형과-MaaS.md) — 이동 니즈 기반 여정 최적화 MaaS 개념과 차량 호출·공유·마이크로모빌리티·대중교통 4유형, 매칭형(우버·타다) vs 검색형(카플랫) 서비스 구분, Citymapper 사례
- [무손실 음원 서비스 UX](concepts/무손실-음원-서비스-UX.md) — 하이레즈 음원 이용자 실태 조사로 본 서비스 학습 비용 문제, 음질 차이보다 '좋다고 하니 듣는' 동기와 경제·시간적 여유를 회복한 중장년층이라는 핵심 타깃
- [서비스 디자인 사례 — JetBlue](concepts/서비스-디자인-사례-JetBlue.md) — 서비스 여정 전체 설계·언행일치 브랜딩·셀프 서비스+인적 지원 조합으로 고객만족 1위 달성한 JetBlue 사례
- [서비스 디자인 사례 — 나이키플러스](concepts/서비스-디자인-사례-나이키플러스.md) — GPS 러닝 데이터 서비스 Nike+의 본질을 목표 관리·데이터 관리·커뮤니티 3요소 결합의 Goal Management Service로 규정한 pxd 분석
- [서비스 디자인 사례 — 더담다](concepts/서비스-디자인-사례-더담다.md) — 말기암 환자·가족의 정서적 고립과 상호 부담을 교환일기 컨셉으로 해소한 서비스 디자인, 2014 Core77 Design Award Service 부문 Winner 수상
- [서비스 디자인 사례 — 리니어블](concepts/서비스-디자인-사례-리니어블.md) — 비콘 기반 미아방지 웨어러블 밴드·앱, pxd와의 앱 2.0 리뉴얼 협업 사례
- [서비스 디자인 사례 — 모두의 가드닝](concepts/서비스-디자인-사례-모두의-가드닝.md) — 서울시립대 인근 방치 화단을 학생과 주민이 함께 가꾸는 게릴라 가드닝 재해석 커뮤니티, 제공자-수혜자 동일 구조로 지속성을 노린 산학협력 서비스 디자인 사례
- [서비스 디자인 사례 — 오사카 회전초밥 부페](concepts/서비스-디자인-사례-오사카-회전초밥-부페.md) — 좌석 배정·음식 배분·문화적 소비 습관을 통해 손님 만족을 해치지 않으면서 이윤을 남기는 오사카 회전초밥 부페의 서비스 설계 트릭
- [서비스 디자인 사례 — 카붐](concepts/서비스-디자인-사례-카붐.md) — 결과물이 아닌 주민·자원봉사자의 경험을 디자인하는 미국 비영리 놀이터 건설 단체, 디자인 데이·빌드 데이로 단 하루 만에 완공하는 참여형 프로세스
- [서비스 디자인 사례 — Podo Camera](concepts/서비스-디자인-사례-Podo-Camera.md) — Kickstarter Staff Pick 선정 블루투스 카메라 Podo, pxd·Podo Labs 1개월 협업으로 본 UI·GUI 통합 프로토타이핑 사례
- [서비스 디자인 씽킹](concepts/서비스-디자인-씽킹.md) — Stickdorn·Schneider의 서비스 디자인 5원칙을 재해석한 "서비스 디자인 − Evidencing = UX, UX − Holistic = UI" 공식
- [서비스 디자인 툴킷](concepts/서비스-디자인-툴킷.md) — Customer Centered Service Design Toolkit(Process Sheet·Check List·Method Card) 리뷰, pxd 더블 다이아몬드와의 비교
- [서비스디자인 글로벌 컨퍼런스 2011](concepts/서비스디자인-글로벌-컨퍼런스-2011.md) — 2011년 샌프란시스코 Service Design Global Conference 참관기, Brandon Schauer(Adaptive Path)의 SAG(Service Anticipation Gap) 개념과 Flow-Nomics 등 4대 전술, 다음날 열린 SDN 한국 협회 발족식
- [서비스디자인 컨퍼런스 2011](concepts/서비스디자인-컨퍼런스-2011.md) — 국내 제1회 서비스디자인 컨퍼런스 참관기, 미하엘 엘호프(KISD)의 People·Process·Physical Evidence와 Sensuality·Sociality·Security, 국내 서비스 디자인 개념 초기 수용기의 혼란
- [서비스 비즈니스의 원칙](concepts/서비스-비즈니스의-원칙.md) — 데이비드 마이스터의 서비스업 운영 원칙: 만족도=지각-기대 공식, 서비스의 가시화, 회사-직원-소비자 삼각구도, 표준화와 맞춤화의 공존
- [서비스 프로세스 효율화](concepts/서비스-프로세스-효율화.md) — 가로수길 백반집 관찰 사례로 본 프로세스 사전 설계·선택지 축소·인력의 유연한 협업을 통한 오프라인 서비스 효율화 원칙, 속도와 정중함의 양립
- [아시아 배달앱과 팬데믹 대응](concepts/아시아-배달앱과-팬데믹-대응.md) — 고젝·스위기·쿠팡이츠의 코로나19 대응 비교: 자체 안전망 강화, Swiggy wallet 전면 디지털 결제 전환, 비대면 결제 기본값화, 위생 커뮤니케이션과 심리적 웰빙
- [앱 개발 사례 — Bucket Labs Phoster](concepts/앱-개발-사례-Bucket-Labs-Phoster.md) — 포토+포스터 합성 앱 Phoster 개발기: 3인 소수팀 협업 구조와 빠른 프로토타이핑으로 해외 앱스토어 상위권에 오른 국내 앱 개발 성공 사례
- [앱 개발 사례 — flur:Wallpaper](concepts/앱-개발-사례-flur-Wallpaper.md) — pxd LeanUX Lab이 Cloudia 이후 짧은 기간에 만든 배경화면 앱 flur:Wallpaper 개발기, Lean UX 이론과 실무의 괴리 및 Apple 앱 심사 리젝션 2회 경험
- [앱 개발 사례 — 멀티 스크린 디바이스](concepts/앱-개발-사례-멀티-스크린-디바이스.md) — pxd LeanUXLab의 2014년 멀티 스크린 디바이스 실험: 듀얼스크린 PDF 뷰어에서 출발해 여러 디바이스를 이어붙여 화면을 확장하는 MVP 기반 단계적 개발과 터치 제스처 동기화 방식
- [앱 개발 사례 — 설날카드](concepts/앱-개발-사례-설날카드.md) — Probetyping Labs(현 Lean UX Lab 전신)의 얼굴인식 기반 설날카드 앱 개발기, 실패한 크리스마스 프로모션 인사이트에서 출발해 2~3주 만에 완성한 MVP 스핀오프 사례
- [음악 스트리밍 앱 첫화면 UX](concepts/음악-스트리밍-앱-첫화면-UX.md) — 멜론·엠넷·벅스 2012년 첫 화면 비교로 본 Music Follower·Music Browser 두 퍼소나 기반 콘텐츠 우선순위 설계
- [유아 성장 트랜지션 학습 앱 아이디어](concepts/유아-성장-트랜지션-학습-앱-아이디어.md) — 다섯 살 딸과의 매미 관찰에서 착안한 pxd 다이어리 아이디어: 슬라이드바로 성장 단계를 조작하면 동식물이 변화하는 모바일 앱 컨셉
- [현재 재생목록 UX 패턴](concepts/현재-재생목록-UX-패턴.md) — 대체식(Replace) vs 축적식(History), 1Tap/2Tap 재생 방식에 따른 스트리밍 뮤직 플레이어의 현재 재생목록 생성 로직
- [Spotify UX와 음악 추천 기술](concepts/Spotify-UX와-음악-추천-기술.md) — Collaborative Filtering·Taste Profile·1,387개 서브 장르로 구현한 스포티파이의 맥락 기반 개인화 UX
- [서비스 디자인 사례 — 제주 원도심 코워킹스페이스](concepts/서비스-디자인-사례-제주-원도심-코워킹스페이스.md) — 옛 기상청 구청사 부지를 스타트업 코워킹스페이스로 전환한 도시재생 프로젝트, 공간·사용자·서비스 3단계 정의 프로세스
- [Service Experience 2013 컨퍼런스](concepts/Service-Experience-2013-컨퍼런스.md) — Adaptive Path 주최 2013 서비스 디자인 컨퍼런스 참관기, Philips Healthcare·Groupon·Richland Library의 조직 내재화 서비스 디자인 실무 사례
- [서비스 디자인 사례 — 버진 아메리카](concepts/서비스-디자인-사례-버진아메리카.md) — Work & Co.와 진행한 항공권 예매 웹 리뉴얼, RWD·즉각 피드백·아바타로 즐거움을 더하고 광고 수익을 포기한 UX 우선 전략

## 디자인 시스템
- [BX 디자인과 브랜드 자산 운용](concepts/BX-디자인과-브랜드-자산-운용.md) — BX 디자이너는 브랜드의 시각적 언어와 사용자 경험을 통합하는 역할
- [Family UI](concepts/Family-UI.md) — 자동차 업계 Family Look에서 차용한 크로스플랫폼 UI 일관성 개념, 미투데이 웹·iOS·안드로이드 앱의 아이콘·레이블·레이아웃·내비게이션 비교 사례
- [GUI 디자인 사례 SKP Tcloud](concepts/GUI-디자인-사례-SKP-Tcloud.md) — SK플래닛 N-스크린 클라우드 스토리지 Tcloud의 Web·iPhone·Android·iPad 크로스플랫폼 GUI 디자인 프로젝트 사례, GUI Lab·UI Lab 역할 분담
- [GUI 디자인 사례 T smart learning](concepts/GUI-디자인-사례-T-smart-learning.md) — pxd·SK텔레콤·청담러닝의 국내 최초 Tablet PC 기반 스마트 교육 플랫폼 GUI 디자인 프로젝트, Launcher형 학습 Tool App 통합 사례
- [UI 컴포넌트 용어](concepts/UI-컴포넌트-용어.md) — UI 기획자와 디자이너, 개발자가 협업할 때 필요한 공통 언어 (Granularity·Coherence 포함)
- [UI-GUI 협업 프로세스](concepts/UI-GUI-협업-프로세스.md) — 컴포넌트 정의→도구 선택→공동 라이브러리→문서 통합 4단계로 UI팀·GUI팀 협업을 개선하는 프로세스, '디자이너의 git' Abstract 활용
- [다크 UI와 눈의 피로](concepts/다크-UI와-눈의-피로.md) — 2010년대 초 After Effects·Expression Blend 등 영상·그래픽 툴이 어두운 GUI를 기본값으로 채택한 배경(눈의 피로 완화 통념)과 이후 OS 전반 다크 모드로의 확산
- [디자인 가이드 문서 자동화 도구](concepts/디자인-가이드-문서-자동화-도구.md) — 이펙트 포함 경계·좌표·마진값을 자동 추출하는 포토샵 플러그인 Assistor PS로 본 가이드 문서 작업 자동화
- [디자인 시스템](concepts/디자인-시스템.md) — 디자인 원칙, 규격, 재사용 가능한 UI 패턴과 컴포넌트, 코드를 포함하는 종합 세트
- [색상이론과 RYB 색상환](concepts/색상이론과-RYB-색상환.md) — RGB와 RYB 색상 모델의 차이, 이텐의 색상환을 디지털 환경에서 구현하는 방법
- [조형의 요소와 원리](concepts/조형의-요소와-원리.md) — 디자인을 구성하는 재료(요소)와 배합 방법(원리)
- [피그마 프로토타이핑](concepts/피그마-프로토타이핑.md) — 피그마의 프로토타이핑 기능으로 인터랙티브 시뮬레이션 제작
- [스큐어모피즘](concepts/스큐어모피즘.md) — 이전 매체의 형태를 새 매체에서 유지하는 GUI 디자인 양식, 애플 iOS 7 플랫 전환의 배경
- [스플래시 화면과 브랜드 아이덴티티](concepts/스플래시-화면과-브랜드-아이덴티티.md) — Adobe Photoshop·Illustrator 릴리즈 역사로 본 스플래시 화면의 개별 아이덴티티 이미지에서 CS 통일 폰트+컬러로의 전환, 브랜딩 접점으로서의 로딩 화면
- [아이콘 디자인과 타이포그래피](concepts/아이콘-디자인과-타이포그래피.md) — 폰트 기반 아이콘 제작법과 Family Look 균형의 원리
- [앱 아이콘 색상 전략](concepts/앱-아이콘-색상-전략.md) — 아이폰 앱 아이콘 색상 사용 경향 분석: 파란색(정보·유틸), 빨간색(게임), 브랜드 아이덴티티 우선 원칙
- [포털 사이트 기념일 로고 이벤트](concepts/포털-사이트-기념일-로고-이벤트.md) — **구글 두들**에서 착안한 2010년 어린이날 국내 포털(네이버·다음·파란 등) 로고 이벤트 비교, 클릭 시 반응 3유형(검색결과·캠페인·정보없음)
- [UI 가이드라인](concepts/UI-가이드라인.md) — Apple HIG·Material Design 등 UI 가이드라인을 효과적으로 읽고 활용하는 방법

## UI/인터랙션 설계
- [UI 디테일 비교와 지속적 개선](concepts/UI-디테일-비교와-지속적-개선.md) — iPhone4·iPod Touch 화면 비교로 본 레이블·아이콘·스크롤바 등 미세 인터페이스 디테일 개선, 가이젠(改善) 관점의 지속적 UI 개선 태도
- [UI 커스터마이징](concepts/UI-커스터마이징.md) — 아이팟 클래식 UI 재현 앱 사례로 본 '열린 디자인'(사용자 참여)과 '닫힌 디자인'(전문가 설계) 사이의 긴장, 프로슈머 흐름과의 연결
- [색맹을 고려한 UI 색상 설계](concepts/색맹을-고려한-UI-색상-설계.md) — 단일 색상 신호(visual cue)의 위험성과 아이콘·텍스트 병행, 명도 대비를 통한 색맹 접근성 개선 원칙
- [썸네일 리스트 UI 패턴](concepts/썸네일-리스트-UI-패턴.md) — 고정영역·가변영역, 줄이기·자르기 분류와 탐색(Browse)·발견(Discover) 맥락별 최적 썸네일 표시 방식
- [안드로이드 내비게이션 룰](concepts/안드로이드-내비게이션-룰.md) — HOME(상태 보존)과 BACK(Activity 종료) 버튼의 근본적 동작 차이와 서드파티 앱들의 비일관성 문제
- [안드로이드 위젯 디자인](concepts/안드로이드-위젯-디자인.md) — 유용성-크기 균형, 터치 영역, 숏컷 개념으로 접근하는 안드로이드 위젯 기획 원칙
- [안드로이드 텍스트 쉐도우 효과](concepts/안드로이드-텍스트-쉐도우-효과.md) — 포토샵 텍스트 그림자를 안드로이드 코드로 옮기는 4대 변수(Color·X/Y Offset·Radius)와 pxd GUI 가이드 표기 방식
- [암묵적 상호작용과 맥락 기반 알림](concepts/암묵적-상호작용과-맥락-기반-알림.md) — 버스 도착 사실을 정확한 타이밍에 알려 명시적 조작 없이 의도를 처리하는 geofence 기반 맥락 인식 알림 사례
- [Pull to Refresh](concepts/Pull-to-Refresh.md) — 화면을 아래로 당겨 새로고침하는 모바일 제스처 UI 패턴, Loren Brichter의 Tweetie에서 시작
- [원형 디스플레이 UI](concepts/원형-디스플레이-UI.md) — 스마트워치 등 원형 디스플레이의 조형 특성(연속성·집중성·현저성)과 글랜서빌리티 기반 UI 설계 가이드라인
- [위젯 독 UI 패턴](concepts/위젯-독-UI-패턴.md) — 모바일 홈 화면·데스크탑 위젯 트레이·TV 위젯 바·게임 인벤토리를 아우르는 위젯 보관함 설계, 배치 방향·표현 방식·화면 점유 범위 변수와 탐색·실행·관리 용이성 3대 설계 기준
- [SNS 댓글 UI 설계](concepts/SNS-댓글-UI-설계.md) — 시간순 정렬과 상단 더보기 버튼을 기반으로 한 SNS 댓글 네비게이션 설계 원칙
- [약관 동의 UI](concepts/약관-동의-UI.md) — 필수·선택 약관 구분과 최소 입력 원칙을 적용한 약관 동의 화면 설계
- [어피니티 버블](concepts/어피니티-버블.md) — pxd가 개발한 업무 도구로, 다수의 텍스트 데이터를 시각적으로 분류
- [옵티미스틱 UI](concepts/옵티미스틱-UI.md) — 서버 응답을 기다리지 않고 즉시 성공 상태를 표시하는 체감 성능 최적화 패턴
- [윈도우 7 디자인 원칙](concepts/윈도우-7-디자인-원칙.md) — 2008년 PDC 발표 Windows 7 디자인팀의 7가지 원칙(Reduce Concepts·Small Things Matter·Delight·Solve Distractions·Time Matters·Full Lifecycle·Look and Do)과 태스크바 단순화·피츠의 법칙 활용 등 구체 사례
- [윈도우 8 모던 UI](concepts/윈도우-8-모던-UI.md) — Tile·Live Tile·Charm bar·Semantic Zoom 등 Windows 8 Modern UI 구성요소와 Desktop UI 공존이 낳은 이질감 한계
- [잠금 화면 해제 UX](concepts/잠금-화면-해제-UX.md) — slide to unlock의 오조작 방지 목적과 아이폰·안드로이드 인체공학 차이, 아이패드2 스마트 커버의 물리적 잠금 해제 대안
- [지도 UI 드래그 반응성](concepts/지도-UI-드래그-반응성.md) — 드래그 중 맵타일 실시간 갱신 여부로 갈리는 네이버·다음 지도와 구글맵의 체감 반응성 차이
- [3D 지도 내비게이션 제스처](concepts/3D-지도-내비게이션-제스처.md) — Google Earth·NASA World Wind·서울시 3D GIS·V World 비교로 본 Rotate·Tilt 구분과 포인터 중심 vs 카메라 중심 회전축 설계 원리
- [브라우저 마우스 제스처](concepts/브라우저-마우스-제스처.md) — 크롬플러스·네이버 툴바·파이어폭스 Fire Gesture 비교로 본 마우스 제스처 방향 조합과 서비스 간 파편화 문제, 모바일 웹 확장 화두
- [브라우저 UI 셸 디자인](concepts/브라우저-UI-셸-디자인.md) — IE9 리뷰로 본 브라우저 크롬(주소창·탭·버튼) UI 비평, Simplicity vs Space Efficiency·Round Rects·"Let there be light" 등 디테일 설계 원칙
- [차량 UX 디자인 프로세스](concepts/차량-UX-디자인-프로세스.md) — 전동화·라이드셰어링·커넥티드 3대 트렌드와 정보설계 뼈대구조·안전·국가별 제약 중심의 자동차 UX 설계 프로세스, 프로토타입 3목적과 버튼 vs 터치스크린 선택 기준
- [초기 태블릿 PC 경쟁과 사용자 경험 비교](concepts/초기-태블릿-PC-경쟁과-사용자-경험-비교.md) — 2010년 아이패드·갤럭시탭·K-Pad 등 '패드 전쟁' 국면에서 화면 크기 차이가 가정용(아이패드) vs 성인 이동중 사용(갤럭시탭)이라는 용도 분화로 이어진다는 대비 분석
- [카드 UI 디자인과 SNS 시간 소비](concepts/카드-UI-디자인과-SNS-시간-소비.md) — 2014년 카드 타입 UI 확산 트렌드와, '무언가 더 있다'는 암시가 SNS 피드 소비 시간을 잠식하는 피로사회적 역설
- [커넥티드카 음성 UX](concepts/커넥티드카-음성-UX.md) — place-ona 개념으로 본 주행 중 음성 인터랙션 우세 조건과 의사결정 최소화·화면 보조채널 설계 원칙
- [키보드 단축키와 인체공학](concepts/키보드-단축키와-인체공학.md) — 윈도우 Ctrl과 맥 Command의 조합키 위치 차이가 손가락에 미치는 부담, KeyTweak 키 매핑 교정과 에브리데이 UX
- [키오스크 UI 설계](concepts/키오스크-UI-설계.md) — 큰 화면, 터치 입력, 공개 장소라는 키오스크 특성에 맞는 UI 설계
- [탠저블 인터랙션 프로토타이핑](concepts/탠저블-인터랙션-프로토타이핑.md) — 장난감 자동차 바퀴에 전도성 점토로 접점을 만들어 아이패드 정전식 터치스크린으로 위치·방향을 인식, 지도 로드뷰와 실시간 연동한 물리적 사물 기반 인터랙션 프로토타입 실험
- [한글 입력 설계](concepts/한글-입력-설계.md) — 자소 빈도 분석에 기반한 한글 자판 설계, TV 환경 유전 알고리즘 키보드 최적화, 어린이 학습용 키보드 사례 포함
- [모바일 제스처 UI](concepts/모바일-제스처-UI.md) — Tap·Swipe·Long Tap 등 모바일 동영상 플레이어 제스처 패턴 비교
- [모바일 데이터 자동 인식](concepts/모바일-데이터-자동-인식.md) — iOS Data Detector의 전화번호·날짜·장소·링크 자동 감지와 탭 가능한 인터랙션으로 변환하는 맥락 인식 기반 UX
- [모바일 결제 서비스 UX](concepts/모바일-결제-서비스-UX.md) — 모바일결제 성공의 두 조건(편의성·커버리지)과 지갑 형태별 사용자 목표 세분화, 삼성페이 마그네틱 병행 지원 전략
- [모바일 브라우저 탭 전환 UI](concepts/모바일-브라우저-탭-전환-UI.md) — Tab·Swipe·Visual Tab·커스텀 제스처 4가지 탭 전환 방식 비교와 인터랙션 충돌 회피 원칙
- [모바일 앱 설정 UI 패턴](concepts/모바일-앱-설정-UI-패턴.md) — iOS HIG의 settings/configuration options 구분과 "80% 니즈 집중" 원칙, pull-to-reveal 숨김 UI, 네이버·다음 지도 반례
- [아이패드 UI 사용성](concepts/아이패드-UI-사용성.md) — Nielsen Norman Group의 2010년 초기 아이패드 사용성 테스트: 손가락 크기 맞춤 터치 존·공통 네비게이션 키 제안, '비즈니스 컴퓨터'에서 '레저 컴퓨터'로의 진단
- [낙서 도구의 디지털화](concepts/낙서-도구의-디지털화.md) — 포스트잇·노트 등 아날로그 낙서 도구와 디지털 대응 제품(민트패드·라이브스크라이브·태블릿 PC) 비교로 본 확장성·즉흥성 vs 보관·공유 편의성 트레이드오프
- [스타일러스 펜 필기 UX](concepts/스타일러스-펜-필기-UX.md) — 2010년 아이패드 출시 직후 Pogo Sketch·BOXWAVE·DAGI 3종 스타일러스 펜 비교, 팁 재질에 따른 필기감 차이와 팜 리젝션(손목 접촉 방지) 부재 문제
- [모바일 웹 리디자인 사례 — 네이버 블로그](concepts/모바일-웹-리디자인-사례-네이버-블로그.md) — 아이패드 대응 네이버 블로그 리디자인으로 본 가독성(DPI 환산 오류)·이탈률(포고스티킹)·RSI 세 가지 모바일 웹 문제와 해법
- [금융 앱 UX](concepts/금융-앱-UX.md) — 주식 초보자를 위한 MTS 재설계, 종목 탐색·구매·운용 3단계 UX 원칙
- [IoT 제품 디자인](concepts/IoT-제품-디자인.md) — 모니터링-제어-최적화-자율화 4단계와 물리적 인터페이스 계승을 중심으로 한 IoT 제품 UX 설계 원칙
- [IoT 핵심 기술 용어](concepts/IoT-핵심-기술-용어.md) — 센싱(MEMS·GeoFencing)·네트워크(Beacon·Zigbee·5G·IPv6)·서비스 인터페이스(딥러닝·HomeKit·Physical Web) 세 축으로 정리한 IoT 기술 용어 사전
- [Natural User Interface (NUI)](concepts/Natural-User-Interface-NUI.md) — 신체로 직접 인터랙션하는 인터페이스의 역사·9가지 기술 요소·한계
- [감정 반응 UI](concepts/감정-반응-UI.md) — 페이스북 좋아요 → 6감정 Reactions 전환 사례와 디자인 결정 분석
- [에러 메시지 작성 원칙 4H](concepts/에러-메시지-작성-원칙-4H.md) — Human·Helpful·Humorous·Humble, 불만의 순간을 기쁨으로 바꾸는 에러 메시지 4원칙
- [햄버거 메뉴 UI](concepts/햄버거-메뉴-UI.md) — 햄버거 메뉴 찬반 논쟁과 탭 바 대안, YouTube 개선 사례
- [확인 취소 버튼 배치](concepts/확인-취소-버튼-배치.md) — OK/취소 버튼 위치를 정하는 5가지 근거(OS 컨벤션·손 조작성·시선 이동·하드키 심리적 공통성·물리적 배치)와 단일 규칙으로 통일하기 어려운 이유
- [파일 업로드 UX 패턴](concepts/파일-업로드-UX-패턴.md) — 대용량 첨부 파일 정책과 인지 가시화 설계
- [팝업 UI 패턴](concepts/팝업-UI-패턴.md) — 확인·알림·입력·옵션 4대 팝업 유형과 인터럽트/이벤트 구분, 되돌림 가능성 기준으로 본 팝업 노출 설계 원칙
- [패럴랙스 스크롤링](concepts/패럴랙스-스크롤링.md) — 디즈니 멀티플레인 카메라에서 유래한 레이어별 스크롤 속도 차등 기법, Flash에서 CSS3/JS 구현으로 전환
- [플랫폼 UI 컨벤션 불일치](concepts/플랫폼-UI-컨벤션-불일치.md) — 안드로이드 체크박스 디자인 실패 사례로 본, OS 표준 컴포넌트를 임의로 재해석할 때 발생하는 플랫폼 UI 컨벤션 불일치 문제
- [로그인 UI 패턴](concepts/로그인-UI-패턴.md) — ID·이메일·소셜·전화번호 로그인 방식 비교와 UX 트레이드오프
- [학습 도구 UX](concepts/학습-도구-UX.md) — 반복학습·손쓰기·연상기억법 등 암기 도구의 UX 설계
- [대중교통 앱 UX](concepts/대중교통-앱-UX.md) — 익숙한 경로·낯선 경로 두 맥락 분석으로 본 카카오지하철 앱 UX 개선점: 발견 가능성·정보 단절·경로 이탈 감지; 인도네시아 BlueBird·Grab·GO-JEK 차량 공유 앱의 현지화 UX
- [동영상 플레이어 UI 패턴](concepts/동영상-플레이어-UI-패턴.md) — 2010년 유튜브 플레이어 리디자인으로 본 컨트롤 바 활성/비활성 상태 분리, 커서 이동 등 암묵적 신호로 UI 노출을 조정하는 컨텍스트 기반 컨트롤 노출 패턴
- [인터랙티브 내러티브와 비선형 스토리텔링](concepts/인터랙티브-내러티브와-비선형-스토리텔링.md) — 사용자 선택이 스토리라인을 결정하는 비선형 서사 형식, 넷플릭스 밴더스내치 UI 분석
- [인터랙티브 매거진과 모션그래픽 출판](concepts/인터랙티브-매거진과-모션그래픽-출판.md) — 2010년 Alexx Henry의 VIV Mag 아이패드 데모로 본 태블릿 모션 매거진 형식과 출판업계 모션그래픽 디자이너 수요 진단
- [버튼 인풋과 음성 인풋 비교](concepts/버튼-인풋과-음성-인풋-비교.md) — 인풋·아웃풋 대칭성·탐색 가능성·정확성·타이밍 제약·업데이트 가시성 5축으로 본 버튼(터치)과 음성 인풋의 근본적 차이
- [AI 스피커 VUX 설계 원칙](concepts/AI-스피커-VUX-설계-원칙.md) — 음성 스피커가 유리한 5가지 시나리오와 대화 핑퐁·피드백·일관된 스타일·선판단 후확인·개인화·짧은 안내로 구성된 VUX 설계 6원칙
- [메가 드롭다운 메뉴](concepts/메가-드롭다운-메뉴.md) — 커서 이동 방향을 삼각형 영역으로 예측해 딜레이 없이 서브메뉴를 전환하는 아마존 메가 드롭다운 메뉴 분석과 jQuery-menu-aim 구현
- [인터랙션 패턴 라이브러리](concepts/인터랙션-패턴-라이브러리.md) — pxd TFD 과제에서 출발해 ixpatterns.com으로 공개한 인터랙션 패턴 위키, WIMP 기반 터치 패턴과 제스처·음성·MR 기반 차세대 패턴의 2층 분류 체계
- [앱 디자인 레퍼런스 큐레이션 도구](concepts/앱-디자인-레퍼런스-큐레이션-도구.md) — grappic(위트스튜디오)의 유저 투표 기반 앱 GUI 크라우드소싱 큐레이션, Pic Board·Street·My Pic 3화면 구조
- [Design Hacking과 웹사이트 리디자인](concepts/Design-Hacking과-웹사이트-리디자인.md) — 클라이언트 스크립트로 타사 사이트 UX를 즉석 개선·시연하는 pxd 블로그 시리즈, 다음 뉴스 자동요약 사례로 본 기본값 설계와 GoodUI 멀티컬럼 회피 원칙
- [페이지네이션 UI 패턴](concepts/페이지네이션-UI-패턴.md) — Daum·네이버·구글·야후US의 페이지 내비게이션 UI 비교로 본 디자인적 일관성 vs 논리적 일관성, 피츠의 법칙 기반 버튼 크기·레이블 설계 원칙
- [AR 번역 앱과 음성 받아쓰기 서비스](concepts/AR-번역-앱과-음성-받아쓰기-서비스.md) — 2011년 초 등장한 Dragon Dictation 음성-텍스트 변환과 Word Lens 카메라 실시간 시각 번역, 언어를 다른 입력 채널로 풀어낸 두 초기 사례

## 웹 개발
- [자료구조 기초](concepts/자료구조-기초.md) — 연결 리스트·이중 연결 리스트·이진 탐색 트리·힙·트라이 등 "노드" 개념 중심의 자료구조 확장, 각 구조의 삽입·삭제·탐색 시간복잡도 비교
- [증분 정적 재생성 (ISR)](concepts/증분-정적-재생성-ISR.md) — Next.js에서 SSG와 SSR의 중간 지점을 메워 빌드 이후에도 페이지 단위로 주기적 재생성을 가능케 하는 렌더링 전략
- [CSS Flexbox](concepts/CSS-Flexbox.md) — float를 대체하는 현대적 CSS 레이아웃 방식, flex-container와 flex-item 속성 체계
- [CSS Grid](concepts/CSS-Grid.md) — display: grid 기반 2차원 격자 레이아웃, grid-template-areas와 IE 지원 한계
- [현대 CSS 기능](concepts/현대-CSS-기능.md) — Container Query·Subgrid·Cascade Layers·:has() 선택자·View Transitions 등 디자인 시스템 시대의 CSS 신기능
- [CSS 폰트 기능 설정](concepts/CSS-폰트-기능-설정.md) — font-feature-settings로 OpenType의 합자·스몰캡·스와시·tabular-nums 제어
- [DFS 알고리즘과 재귀적 콘텐츠 파서](concepts/DFS-알고리즘과-재귀적-콘텐츠-파서.md) — 약관 JSON을 깊이 우선 탐색(DFS) 재귀 함수로 순회해 렌더링하는 방식으로 버전별 조건 분기 기술 부채를 해소한 콘텐츠 파서 사례
- [SVG 차트 구현](concepts/SVG-차트-구현.md) — 라이브러리 없이 polyline·circle로 라인 차트와 파이 차트 직접 구현
- [웹 컴포넌트](concepts/웹-컴포넌트.md) — 웹 표준 HTMLElement·Shadow DOM 기반 재사용 컴포넌트 제작
- [React Native와 Expo 앱 개발](concepts/React-Native와-Expo-앱-개발.md) — 디자이너가 React Native + Expo로 직접 만든 날씨/대기질 앱 개발 경험, 엣지 케이스·외적 제약·유통의 중요성
- [React 컴포넌트 성능 최적화](concepts/React-컴포넌트-성능-최적화.md) — React.memo·useMemo·useCallback 메모이제이션, Dynamic Import, 데이터 fetching 전략과 메모이제이션 남용 주의점
- [React 컴포넌트 패턴](concepts/React-컴포넌트-패턴.md) — Container/Presentational, Compound, HOC 세 가지 React 디자인 패턴
- [SOLID 원칙과 프론트엔드](concepts/SOLID-원칙과-프론트엔드.md) — 컴포넌트·훅 단위 프론트엔드 설계에 적용하는 SOLID 5원칙: SRP·OCP·LSP·ISP·DIP
- [오픈소스 라이브러리 리스크 관리](concepts/오픈소스-라이브러리-리스크-관리.md) — faker.js·colors.js 사태로 본 메인테이너發 공급망 리스크와 버전 고정·모니터링·후원 등 대응 원칙
- [오픈소스 라이선스](concepts/오픈소스-라이선스.md) — MIT·Apache·GPL·LGPL·MPL·BSD 비교, Permissive vs Copyleft 분류
- [CSS 로딩 애니메이션](concepts/CSS-로딩-애니메이션.md) — 처리 시간 동안 시스템 작동 중임을 알려주는 CSS 기반 피드백 요소
- [JWT 인증과 인가](concepts/JWT-인증과-인가.md) — Authentication(신원 검증)과 Authorization(권한 부여)의 차이, JWT 구조와 토큰 기반 인증 메커니즘
- [프론트엔드 보안](concepts/프론트엔드-보안.md) — XSS·CSRF·클릭재킹·민감 데이터 노출 등 프론트엔드 개발 시 주요 보안 위협과 Next.js 환경 대응 방법
- [Safari 포커스 관리](concepts/Safari-포커스-관리.md) — Safari의 비입력 요소 포커스 비활성화로 인한 스크롤 튕김 버그와 tabindex="-1" 해결책
- [GitHub Actions와 Slack 연동](concepts/GitHub-Actions와-Slack-연동.md) — GitHub CI/CD 서비스와 Slack 알림 연동
- [GitHub 칸반보드](concepts/GitHub-칸반보드.md) — GitHub Projects를 활용한 개발팀 업무 시각화와 진척 관리
- [JavaScript Date 라이브러리](concepts/JavaScript-Date-라이브러리.md) — Moment.js, Day.js, date-fns 세 가지 날짜 라이브러리 비교
- [PWA 프로그레시브 웹 앱](concepts/PWA-프로그레시브-웹-앱.md) — 웹 기술로 네이티브 앱 경험을 제공하는 방식, manifest.json과 Service Worker 기반
- [React TypeScript 패턴](concepts/React-TypeScript-패턴.md) — React.FC의 문제점(암시적 children, defaultProps 이슈)과 일반 함수 선언 방식 권장 패턴
- [TypeScript 타입 시스템 기초](concepts/TypeScript-타입-시스템-기초.md) — tsc 컴파일 절차와 기본 타입(boolean·number·배열·any)·interface를 통한 형태 계약 정의 등 TypeScript 입문 개념
- [React 상태 관리](concepts/React-상태-관리.md) — Jotai 아토믹 상태 관리 라이브러리와 React 상태 관리 라이브러리 비교
- [React 폼 제어](concepts/React-폼-제어.md) — React Hook Form 기반 폼 제어, 제어/비제어 컴포넌트, useForm·register·watch·Controller 핵심 API
- [SCSS 모듈 시스템](concepts/SCSS-모듈-시스템.md) — @import 대체 문법인 @use와 @forward로 네임스페이스 기반 SCSS 모듈 관리
- [Intersection Observer API](concepts/Intersection-Observer-API.md) — 뷰포트와 타깃 요소의 교차(노출) 여부를 비동기 감시하는 Web API, `scroll` 이벤트의 성능 부담을 대체하는 Lazy load·인피니티 스크롤 구현 수단
- [Resize Observer API](concepts/Resize-Observer-API.md) — DOM 요소의 크기 변화를 비동기적으로 감시하는 Web API, `resize` 이벤트의 메모리 누수·throttle 문제를 해결하는 성능 친화적 대안
- [View Transitions API](concepts/View-Transitions-API.md) — 브라우저가 화면 전환 전후 상태를 자동 감지해 자연스러운 애니메이션을 생성하는 웹 표준 API
- [WebGL과 3D 웹](concepts/WebGL과-3D-웹.md) — 브라우저에서 3D 그래픽을 구현하는 WebGL과 Babylon.js 프레임워크
- [구글 캘린더 매쉬업과 리마인더 자동화](concepts/구글-캘린더-매쉬업과-리마인더-자동화.md) — 구글 캘린더의 SMS 알림과 구독형 공개 캘린더를 조합해 별도 앱 없이 우산 리마인더를 구현한 초기 매쉬업 사례
- [기획 디자인 개발 간극](concepts/기획-디자인-개발-간극.md) — 정적인 기획·디자인 시안과 동적인 구현 흐름 사이의 간극, 디자인 시스템 엣지 케이스와 "UI는 상태의 집합"이라는 관점
- [딥링크](concepts/딥링크.md) — URL이나 값을 호출했을 때 앱을 실행하거나 앱 내 특정 화면으로 이동시키는 기술
- [반응형 웹 디자인](concepts/반응형-웹-디자인.md) — 하나의 HTML로 다양한 해상도에 대응하는 RWD 기법, Fluid Grids·Flexible Images·CSS3 Media Queries와 960/1140 Grid 논쟁
- [브라우저 기반 개발 환경](concepts/브라우저-기반-개발-환경.md) — Code-Server로 VS Code를 브라우저에서 구동해 장소·기기 제약 없이 원격 개발하는 방법
- [Docker와 개발 환경 표준화](concepts/Docker와-개발-환경-표준화.md) — node 버전 파편화 해소를 위한 Docker 도입, nvm과의 비교, Windows WSL2 진입 장벽
- [브라우저 렌더링 파이프라인](concepts/브라우저-렌더링-파이프라인.md) — DOM·CSSOM·렌더 트리·레이아웃·페인트·컴포지트 6단계와 리플로우·리페인트 최소화 전략
- [크로스브라우저 호환성](concepts/크로스브라우저-호환성.md) — 액티브엑스 등 특정 브라우저 전용 기술 의존과 조용히 실패하는(silent failure) UX의 위험성, 2011년 다나와 IE 종속 사례
- [로컬터널과 개발환경](concepts/로컬터널과-개발환경.md) — localtunnel로 로컬 개발 서버를 외부에서 접근 가능하게 터널링하는 방법
- [성능 최적화와 기획 디자인 협업](concepts/성능-최적화와-기획-디자인-협업.md) — 성능은 개발 단계가 아닌 기획·디자인 단계에서 결정된다는 관점, 무한스크롤·과도한 실시간 갱신·이미지·웹폰트 남용 등 성능 함정과 협업 체크리스트
- [순서도와 프로그래밍 사고](concepts/순서도와-프로그래밍-사고.md) — 워크플로우와 과정을 도형으로 표현하는 시각화 도구
- [오픈 그래프 프로토콜](concepts/오픈-그래프-프로토콜.md) — 웹 페이지의 HTML head 내 메타 태그로 정보를 전달하는 프로토콜
- [웹 아키텍처](concepts/웹-아키텍처.md) — 웹서버와 WAS의 구조와 역할
- [웹 인증과 쿠키](concepts/웹-인증과-쿠키.md) — 서브도메인 쿠키 공유와 iframe 기반 크로스 서비스 세션 유지 기법
- [웹 접근성과 WAI-ARIA](concepts/웹-접근성과-WAI-ARIA.md) — 모든 사용자가 웹 서비스를 동등하게 이용할 수 있도록 하는 설계 원칙
- [웹 페이지 로딩 속도](concepts/웹-페이지-로딩-속도.md) — 싸이월드 블로그 vs 모바일 페이지의 HTTP 요청 수 비교(309개 vs 37개)로 본 2011년 로딩 속도와 UX의 관계, "기다리게 하지 않는 것"이 가장 근본적인 사용자 경험이라는 원칙
- [이미지 최적화](concepts/이미지-최적화.md) — 포맷(webp/avif)·규격 정의·레이지 로딩 세 축으로 초기 로딩 성능을 개선하는 기법, NILE 서비스 개선 사례
- [코어 웹 바이탈과 SEO](concepts/코어-웹-바이탈과-SEO.md) — LCP·FID·CLS 세 지표로 측정하는 Google 페이지 경험 기준, 검색 순위와 비즈니스 성과에 직접 연결
- [테스트 코드 작성 전략](concepts/테스트-코드-작성-전략.md) — the-zero 프로젝트 백엔드 개발 경험으로 본 테스트 피라미드(정적 분석-유닛-통합-E2E)와 신뢰성·가독성을 위한 테스트 코드 작성 전략
- [피그마 브라우저 렌더링 차이](concepts/피그마-브라우저-렌더링-차이.md) — 논리적 픽셀 vs 물리적 픽셀, 소수점 픽셀 처리, 폰트 렌더링, 색 표현 방식 네 가지로 본 피그마-브라우저 UI 불일치 원인
- [Next.js API 프록시 레이어](concepts/Next.js-API-프록시-레이어.md) — App Router API Routes를 프론트-백엔드 사이에 두어 CORS·백엔드 URL 노출·httpOnly 쿠키 기반 인증 토큰 관리 문제를 해결하는 BFF 패턴
- [Next.js 레이아웃과 템플릿](concepts/Next.js-레이아웃과-템플릿.md) — layout vs template의 동작 원리 차이와 고급 최적화
- [Next.js 사이트맵 자동화](concepts/Next.js-사이트맵-자동화.md) — next-sitemap 패키지로 sitemap.xml·robots.txt를 빌드 파이프라인에 자동 편입하는 SEO 패턴
- [Nuxt.js 프레임워크](concepts/Nuxt.js-프레임워크.md) — Vue.js 기반 SSR 프레임워크, 파일 기반 라우팅·자동 코드 분할·SCSS 전역 스타일 연동
- [Socket.IO와 실시간 통신](concepts/Socket.IO와-실시간-통신.md) — WebSocket 기반 양방향 실시간 통신 라이브러리
- [커스텀 테이블 컴포넌트](concepts/커스텀-테이블-컴포넌트.md) — Next.js+TypeScript 기반 커스텀 테이블 구현 원칙
- [스토리북 컴포넌트 개발](concepts/스토리북-컴포넌트-개발.md) — Storybook addon 활용 및 컴포넌트 문서화 워크플로우
- [Vue3 마이그레이션](concepts/Vue3-마이그레이션.md) — Vue2 → Vue3 전환 시 제거된 기능(키코드 수식어·필터·이벤트 버스 $on/$off/$once)과 대체 패턴
- [슬랙봇 개발](concepts/슬랙봇-개발.md) — Slack RTM API와 외부 오픈 API(날씨, 대기질) 연동 슬랙봇 구현 방법
- [Closed API와 Augmented Browsing](concepts/Closed-API와-Augmented-Browsing.md) — 이름만 '오픈'인 API가 필요한 정보를 내주지 않을 때 브라우저 확장으로 공개 정보를 가공해 보완하는 2011년 티스토리 팀블로그 저자별 모아보기 구현 사례
- [Cheerio와 Node.js 웹 스크래핑](concepts/Cheerio와-Node.js-웹-스크래핑.md) — jQuery와 유사한 API로 HTML을 파싱·추출하는 Node.js 웹 스크래핑 모듈, axios + cheerio.load() 사용 흐름
- [FED 프론트엔드 UX 워크플로우](concepts/FED-프론트엔드-UX-워크플로우.md) — The0 프로젝트로 본 프론트엔드 개발자의 애자일 UX 프로세스
- [디자이너 개발자 협업](concepts/디자이너-개발자-협업.md) — 멀티탭 논란 사례: 디자인은 질문, 개발자의 구현 의지가 혁신을 만든다
- [Vite와 React 빌드 환경](concepts/Vite와-React-빌드-환경.md) — CRA deprecated 이후 ESM 기반 빠른 빌드 도구 Vite로 React 환경 구성하기
- [XAML과 WPF](concepts/XAML과-WPF.md) — 닷넷 기반 UI 마크업 언어 XAML과 WinForm 후속 GUI 프레임워크 WPF, 디자인/개발 영역 분리 구조
- [한글 웹폰트](concepts/한글-웹폰트.md) — 애플고딕 가독성 문제와 iOS 4.3 시스템 폰트 교체 제한을 모바일 사파리 CSS3 @font-face 웹폰트로 우회한 나눔고딕 적용 사례
- [CSR과 SSR 렌더링 방식](concepts/CSR과-SSR-렌더링-방식.md) — SPA·MPA 관계를 바탕으로 한 CSR·SSR 장단점 비교와 Nuxt·Next.js 등 두 방식을 절충하는 프레임워크 사례
- [AI 인용 최적화 콘텐츠 구조](concepts/AI-인용-최적화-콘텐츠-구조.md) — RAG 기반 AI 검색이 페이지 전체가 아닌 특정 구간을 인용한다는 전제 아래 헤딩·문단·FAQ 구조와 구체적 통계·출처 명시로 인용률을 높이는 GEO 콘텐츠 설계 원칙
- [React 서버 중심 렌더링 생태계 동향](concepts/React-서버-중심-렌더링-생태계-동향.md) — Server Components·Streaming UI·React Compiler로 대표되는 App Router 이후 React 생태계의 서버 중심 전환과 그 배경
- [기술적 SEO와 크롤링 최적화](concepts/기술적-SEO와-크롤링-최적화.md) — 크롤링→인덱싱→랭킹 3단계와 robots.txt·sitemap·canonical·크롤 예산 관리, 학습용/검색노출용/사용자요청용으로 나뉘는 AI 크롤러 대응 전략
- [구조화 데이터와 JSON-LD](concepts/구조화-데이터와-JSON-LD.md) — Article·Product·BreadcrumbList 등 스키마 타입별 필수 속성과, 검색 순위를 직접 올리지 않고 리치 결과로 CTR을 높이는 간접 SEO 기여 구조
- [package.json과 npm 패키지 배포](concepts/package.json과-npm-패키지-배포.md) — 공통 컴포넌트 패키지 배포 시 dependencies·devDependencies·peerDependencies 역할 구분과 버전 호환성 위임 전략

## AI와 신기술
- [AI·로봇 윤리와 자율 무기](concepts/AI-로봇-윤리와-자율-무기.md) — 앨런 윈필드의 자동화세·로봇 윤리 3대 쟁점과 로날드 아킨의 치명적 자율 무기(킬러 로봇) 논의로 본 AI·로봇 시대의 윤리적 책임 소재
- [소셜 로봇 디자인](concepts/소셜-로봇-디자인.md) — Hinde의 사회조직 이론과 Norman의 Visceral/Behavioral 디자인 관점으로 본 소셜로봇 정의, Tele-presence Robot 설계 3원칙과 Robot Companion의 Social Distance 실험
- [AI 시대 디자이너의 역할](concepts/AI-시대-디자이너의-역할.md) — HCI 4단계 진화와 AI 시대 디자이너의 4가지 역할 관점: 기술 이해·AI를 위한 디자인·AI와의 협업·AI 윤리
- [AI 모더레이터와 UX 리서치](concepts/AI-모더레이터와-UX-리서치.md) — Outset·Wondering 같은 AI 모더레이터 툴의 강점·한계와 실무 적용 기준
- [AI 비즈니스 전략](concepts/AI-비즈니스-전략.md) — 크리스 더피의 AI 개론서 서평: 좁은/일반/초지능 3단계 분류, 속도·이해·성과·실험·결과의 슈퍼 프레임워크, "AI의 핵심은 사람"이라는 인간-AI 역할 분리 메시지
- [MCP 모델 컨텍스트 프로토콜](concepts/MCP-모델-컨텍스트-프로토콜.md) — Anthropic이 공개한 LLM-외부 시스템 연결 표준, AI 생태계의 USB-C
- [LangChain과 LLM 개발](concepts/LangChain과-LLM-개발.md) — LLM·체인·인덱스·메모리·에이전트로 구성된 LLM 애플리케이션 프레임워크
- [브랜드 디자인과 생성형 AI](concepts/브랜드-디자인과-생성형-AI.md) — Firefly 기반 브랜드 자산 제작과 브랜드 코어 중심의 디렉팅 역량
- [AI 코딩 도구 활용](concepts/AI-코딩-도구-활용.md) — 깃허브 코파일럿, 커서, Claude 등 AI 코딩 도구 실무 활용법과 디자이너의 생성형 AI 적용 사례
- [생성형 AI와 스테이블 디퓨전](concepts/생성형-AI와-스테이블-디퓨전.md) — 스테이블 디퓨전의 디퓨전 프로세스 원리, ComfyUI 노드 기반 워크플로우, 생성형 AI의 민주화 과정
- [AI를 위한 UI 패턴과 UX](concepts/AI를-위한-UI-패턴과-UX.md) — AI 기반 서비스의 새로운 상호작용 방식과 생성형 AI UX 개선 요구 사항
- [에이전틱 AI와 UX](concepts/에이전틱-AI와-UX.md) — 자율적으로 목표를 달성하는 Agentic AI 시대의 6가지 UX 원칙
- [자율주행 UX](concepts/자율주행-UX.md) — 신뢰·예측 가능성·개인 공간을 축으로 하는 자율주행 차량 호출 서비스 UX 설계
- [텍스트 임베딩과 데이터 시각화](concepts/텍스트-임베딩과-데이터-시각화.md) — 텍스트를 고차원 벡터로 변환하여 시각화하는 기술
- [텍스트 분석 도구와 UX 리서치](concepts/텍스트-분석-도구와-UX리서치.md) — NLP·t-SNE 기반 정성 텍스트 자동 군집화로 리서처의 분석 인지 부담을 줄이는 도구 '모라고'
- [프로액티브 AI](concepts/프로액티브-AI.md) — 사용자 요청 없이 맥락을 파악해 먼저 제안·실행하는 AI의 4단계 자동화 수준과 UX 신뢰 설계
- [메타버스와 디지털 현실](concepts/메타버스와-디지털-현실.md) — 김상균 교수의 디지털 현실 개념, 스토리 리빙, 메타버스의 명암과 플랫폼 설계 원칙(빌런·몰입 요소·사회적 문제 대비), 가상 커뮤니티인 메타버스와 화폐·구독 통합 생태계인 멀티버스의 구분(제페토x구찌, 구글 Play Points/Pass 사례)
- [블록체인 UX와 웹3](concepts/블록체인-UX-웹3.md) — 블록체인 기반 탈중앙화 환경의 UX 설계, DeFi UX 원칙, 스테이블코인 유형
- [NFT의 경제적 가치와 소유 개념](concepts/NFT의-경제적-가치와-소유-개념.md) — 홍기훈 교수의 pxd talks 강연: 디지털화와 디지털 트랜스포메이션의 구분, 희소성·내재적 가치·기술적 가치로는 설명 안 되는 NFT 가격, 클럽재-공공재 중간 성격의 소유가 만드는 사회적 가치
- [서울메타위크 2025와 AI 트렌드](concepts/서울메타위크-2025와-AI-트렌드.md) — AI 에이전트·멀티모달AI·생성형AI 창작 UX를 다룬 SMW 2025 컨퍼런스 주요 메시지
- [음성 에이전트와 VUI](concepts/음성-에이전트와-VUI.md) — 구글·시리·빅스비·클로바·누구·알렉사 6종 음성 에이전트의 칫챗 대응 방식 비교와 VUI 정체성 설계
- [알고리즘 경험 디자인](concepts/알고리즘-경험-디자인.md) — AI 추천/매칭 알고리즘을 마주할 때 사용자가 느끼는 당혹감과 UX 디자이너의 역할: 알고리즘 제시 디자인과 명시적/묵시적 피드백을 통한 알고리즘 길들이기
- [머신러닝 학습 방식과 GAN](concepts/머신러닝-학습-방식과-GAN.md) — 지도·비지도·강화 학습 3분류와 생성 모델·분류 모델이 경쟁하며 학습하는 GAN 원리, 온글잎 폰트 생성 사례로 본 남세동 대표의 pxd talks 강연
- [제너레이티브 디자인과 유전자 알고리즘](concepts/제너레이티브-디자인과-유전자-알고리즘.md) — HCI KOREA 2015 이건명 발표, 유전자 알고리즘으로 디자인 생성·평가·반영을 자동화하는 Automated Generative Design, 반지 디자인 사례로 본 취향 기반 추천
- [GEONIQ 산업별 AI 검색 인사이트 리포트](concepts/GEONIQ-산업별-AI-검색-인사이트-리포트.md) — GEONIQ 진단 기준으로 은행·통신·이커머스·OTT·자산운용사·여행숙박 등 업종별 웹사이트를 분석하는 리포트 시리즈, "정보의 양이 아니라 선택 가능한 구조의 부재"가 공통 원인이라는 결론
- [AI 에이전트 시대의 개발자 역할](concepts/AI-에이전트-시대의-개발자-역할.md) — AI 에이전트가 코드를 넘어 PR까지 올리는 시대, '코드 작성자'에서 '시스템 지휘자'로 이동하는 개발자 역할과 T자형 인재, 삼양라운드스퀘어 DXT팀의 AI 주도 개발(맥도날드 프랜차이즈 모델) 사례
- [AX 조직 업무 이해 프레임워크](concepts/AX-조직-업무-이해-프레임워크.md) — AI 전환을 기술 도입이 아닌 조직 진단 문제로 보는 4가지 주제·8가지 축 프레임워크(업무 구조·소통 방식·정보 소스·업무 속도)
- [프롬프트 엔지니어링의 변화](concepts/프롬프트-엔지니어링의-변화.md) — AI가 모호함을 스스로 해석하는 수준에 이르며 "프롬프트를 잘 쓰는 법"보다 "목적을 명확히 정의하는 능력"이 중요해진다는 관점, RAG·AI Agent 개념 정리
- [AX 해자와 컨텍스트 경쟁력 프레임워크](concepts/AX-해자와-컨텍스트-경쟁력-프레임워크.md) — 차별화(Differentiation)와 해자(Moat)를 구분하고, 데이터·업무 방식·의사결정 기준 3축의 컨텍스트를 AI 시대의 진짜 경쟁 우위로 보는 4분면 진단 프레임워크
- [shadcn/ui CLI와 AI 에이전트 연동](concepts/shadcn-ui-CLI와-AI-에이전트-연동.md) — Skills·MCP 서버·Preset 3가지 설정으로 AI 에이전트에게 프로젝트 컨텍스트(설정·최신 문서·디자인 시스템)를 제공하는 shadcn/cli v4 활용법
- [하네스 엔지니어링](concepts/하네스-엔지니어링.md) — AI 에이전트가 안정적으로 작업하도록 컨텍스트·아키텍처 제약·엔트로피 관리 3기둥으로 환경을 설계하는 접근, 프롬프트 엔지니어링(부탁)과 대비되는 강제(harness) 개념
- [AI 협업에서 판단력을 유지하는 워크플로우](concepts/AI-협업에서-판단력을-유지하는-워크플로우.md) — production-to-evaluation shift와 Generation effect로 본 AI 위임의 이해도 저하 문제, 신뢰보다 자신감이 비판적 사고를 지키는 5단계 실천 워크플로우

## UX 커리어
- [GUI 디자이너 직무와 pxd 업무방식](concepts/GUI-디자이너-직무와-pxd-업무방식.md) — 포트폴리오·인터뷰 준비부터 pxd 4D 프로세스·수평적 협업 문화까지, GUI 디자이너의 직무와 입사 경험
- [UX 라이터 직무와 회사 문화](concepts/UX-라이터-직무와-회사-문화.md) — pxd people 연재를 통해 본 UX 라이터의 역할, UX 컨설팅 조직의 구성원 다양성과 협업 구조
- [PM 프로젝트 관리](concepts/PM-프로젝트-관리.md) — 디자인 컨설팅에서 PM 역할의 핵심, 관리(역할)와 리더십(자질)의 구분
- [UX 교육과 대학원](concepts/UX-교육과-대학원.md) — UX를 체계적으로 공부하는 방법과 대학원 선택 가이드, 해외 유학 실전 조언
- [UX 온라인 강의 플랫폼](concepts/UX-온라인-강의-플랫폼.md) — Udemy·Coursera·패스트캠퍼스·Educast 등 온라인 UX 강의 플랫폼별 성격 비교와 학습 유연성 장점
- [UX/UI 면접 준비](concepts/UX-UI-면접-준비.md) — UX/UI 직군 면접에서의 역량 확인과 협업 방식 평가
- [UXer 성과 평가와 연봉협상](concepts/UXer-성과-평가와-연봉협상.md) — 성과 면담은 미래 중심 대화, pxd 키팩터 5가지와 연봉은 미래 기대치라는 협상 원칙
- [UX 포트폴리오](concepts/UX-포트폴리오.md) — 심사자의 경험을 디자인하라: 3개 대표작 15페이지 이내로 승부하는 포트폴리오 원칙
- [UX 커리어 로드맵](concepts/UX-커리어-로드맵.md) — 'Why'에서 출발, UX Sundial·5가지 소프트 스킬·4가지 업무 환경 기반 자기 평가
- [UX 커뮤니티와 스터디 그룹](concepts/UX-커뮤니티와-스터디-그룹.md) — HXA Forum·넥스터즈·페이스북 그룹 등 동호회·스터디 활동을 통한 UX 학습·네트워킹 경로
- [UI 패턴 원칙 도서](concepts/UI-패턴-원칙-도서.md) — UI 패턴과 원칙에 집중한 20권 이상의 참고 도서 목록 (웹·모바일·UX 원칙 분야별 정리)
- [UX 입문 도서](concepts/UX-입문-도서.md) — 방법론·UI·GUI 3축으로 정리한 입문 필독서 11권 (About Face, 새로운 디자인 도구들 등)
- [UX 디자인 운영](concepts/UX-디자인-운영.md) — 유지보수를 넘어 성장까지 만드는 운영 업무, B2B 커스터마이징과 트렌드 선제 제안
- [pxd 블로그 운영](concepts/pxd-블로그-운영.md) — 개인 전문성 성장을 목표로 설계된 pxd story 블로그 10년 운영 철학과 전략
- [pxd 채용 프로세스](concepts/pxd-채용-프로세스.md) — 특정 시즌 공채가 아닌 상시 채용 구조, 서류 심사→실무 면접→임원 면접 3단계 절차와 블로그 정독을 사실상의 자격 요건으로 제시하는 채용 문화
- [비주얼 씽킹과 드로잉 입문](concepts/비주얼-씽킹과-드로잉-입문.md) — 생각을 간단한 그림으로 표현하는 비주얼 씽킹 방법론과 비전공자 드로잉 입문 원칙
- [비즈니스 생산성과 직업관 도서](concepts/비즈니스-생산성과-직업관-도서.md) — 경력 단계별(신입~선임 이상) 비즈니스·생산성·직업관 추천 도서 목록
- [2011 pxd talks 사내 강연 시리즈](concepts/2011-pxd-talks-사내-강연-시리즈.md) — 2011년 8회에 걸쳐 열린 **pxd talks**: 서비스 디자인·퍼소나·Lean UX·데이터 시각화·헬스케어 등 이후 pxd 핵심 방법론이 처음 소개된 사내 강연 시리즈
- [pxd talks 엠블럼 디자인](concepts/pxd-talks-엠블럼-디자인.md) — 사내 교육 프로그램을 TED처럼 브랜드화하기 위해 2012년 GUX디자인센터가 제작한 엠블럼, 말풍선 메타포·황금비·RGB 컬러로 구성한 3개 말풍선 벤다이어그램 구조
- [산학협력 UX 교육 사례](concepts/산학협력-UX-교육-사례.md) — 한경대 음성인식 소재 팀 프로젝트로 본 pxd 실무자 참여형 산학협력 UX 더블다이아몬드 교육 모델
- [소셜 피드백 플랫폼과 교육 경험 디자인](concepts/소셜-피드백-플랫폼과-교육-경험-디자인.md) — 클레비(clebee)를 결합한 pxd 오프라인 교육 워크샵 사례, 실시간 인터랙션과 인터벌 인터랙션의 대비로 본 피드백 아카이빙 설계
- [신규 입사자 온보딩](concepts/신규-입사자-온보딩.md) — UX 관점으로 설계하는 온보딩 여정: Pre-Boarding부터 6개월까지의 경험 설계
- [UX 디자이너 성장과 리더십](concepts/UX-디자이너-성장과-리더십.md) — 디자이너의 성장은 자신이 좋아하는 것을 아는 것에서 출발, 협업 지향 리더십과 조직 문화 형성
- [UX 디자이너의 핵심 역량](concepts/UX-디자이너의-핵심-역량.md) — 문제 해결·학습·공감, UX 디자이너를 구성하는 세 가지 핵심 역량과 각 역량의 입력-처리-출력 구조
- [좋은 동료의 특징](concepts/좋은-동료의-특징.md) — 자기 인지·탐구적 질문·논리적 의견 제시, 디자인 에이전시에서 같이 일하고 싶은 동료의 세 가지 특성
- [직장인 스트레칭과 신체 건강관리](concepts/직장인-스트레칭과-신체-건강관리.md) — 교감신경·부교감신경 균형, 항상성 기반 이완 원리, 사무실에서 실천 가능한 목·어깨·등 스트레칭 방법
- [MMPI-2와 자기이해](concepts/MMPI-2와-자기이해.md) — 스트레스 상황에서 두드러지는 성격 특성을 진단하는 다면적 인성 검사 MMPI-2, MBTI와의 차이와 자기 이해를 통한 소통 방식 개선
- [독서 습관 형성 전략](concepts/독서-습관-형성-전략.md) — 세바시 '책읽기를 말하다' 강연: 장석주의 코이의 법칙과 무용지대용, 서민의 독서 3대 혜택(공감·인내심·논리력), 김봉진의 5원칙으로 본 독서 습관 형성법
- [완벽주의와 심리적 여유](concepts/완벽주의와-심리적-여유.md) — 하지현 교수 pxd talks 강연: 최선·위기의식·완벽주의 3대 강박과 건강한 자기애, 심리적 여유를 위한 5가지 지침
- [창의적 모방과 디자인 성장](concepts/창의적-모방과-디자인-성장.md) — Good artists steal 철학 기반 초급→중급 성장, 파라프레이징·극한 밀기·서로 다른 점 연결 방법론
- [컨퍼런스 연사 도전과 발표 불안 극복](concepts/컨퍼런스-연사-도전과-발표-불안-극복.md) — Call for Speaker 지원부터 발표 트라우마 극복까지, 컴포트 존 확장을 통한 성장 경험
- [프로젝트 레트로스펙티브](concepts/프로젝트-레트로스펙티브.md) — pxd의 프로젝트 회고 방법론, 팀 학습을 통한 조직 지식 축적
- [프로젝트 룸과 협업 공간](concepts/프로젝트-룸과-협업-공간.md) — 프로젝트 팀 전용 물리 공간(칠판·벽·프로젝터·스피커)이 만드는 즉각적 커뮤니케이션·유대감과 사생활 노출·공간 점유 갈등의 명암
- [재택근무 PM 협업](concepts/재택근무-PM-협업.md) — 원격 근무 환경에서의 PM 협업 노하우: 온라인 프로스펙티브·레트로스펙티브, 타임라인 가시화, 정서적 연결
- [UX 엔지니어 직무와 XE 그룹](concepts/UX-엔지니어-직무와-XE-그룹.md) — pxd XE(eXperience Engineering)그룹의 업무 범위·재량근로제 기반 업무 방식·필요 역량으로 본 UX Engineer 직무
- [비폭력대화(NVC)](concepts/비폭력대화-NVC.md) — 마샬 로젠버그의 비폭력대화 모델: 기린/자칼 비유와 관찰-느낌-욕구-부탁 4단계 대화법, pxd talks NVC 교육 실습 사례
- [재능발휘 제도와 사내 취미 지원](concepts/재능발휘-제도와-사내-취미-지원.md) — 직원 취미생활을 회사가 직접 지원하는 pxd 복지 제도, "직원이 행복해야 pxd도 행복하다"는 철학과 꽃꽂이(플라워 클래스) 사례
- [맑은손 공동체와 사내 방문 안마](concepts/맑은손-공동체와-사내-방문-안마.md) — 2014년 pxd가 시범 도입한 시각장애인 안마사 사회적기업 '맑은손 공동체'의 사내 방문 안마 서비스, Enactus 손길 대학생 팀과의 협업으로 이어진 연결
- [지식 포트폴리오](concepts/지식-포트폴리오.md) — 『실용주의 프로그래머』의 지식 포트폴리오 개념: 지식·경험은 소진하는 자산이라는 전제 아래 주기적인 투자·다각화·검토및재조정 등 5원칙과 검색 결과에 대한 비판적 사고
- [pxd 직원 개인 창작 활동과 전시](concepts/pxd-직원-개인-창작-활동과-전시.md) — GUI 디자이너 limho의 개인 그래픽전 '관심사展' 사례로 본 "GUI는 사용자가 사용하는 그래픽"이라는 자기 인식과 pxd 블로그의 업무 외 창작 활동 공유 문화
- [pxd Wii 스포츠대회와 사내 친목 문화](concepts/pxd-Wii-스포츠대회와-사내-친목-문화.md) — Wii·키넥트 모션 게임 기반 사내 체육대회, 승부보다 평소 교류가 적던 동료와 친해지는 계기로서의 조직문화 장치
- [pxd배 사생대회와 공모전 이벤트 운영](concepts/pxd배-사생대회와-공모전-이벤트-운영.md) — 블로그 독자 투표로 대상을 선정하고 경품 추첨으로 참여를 유도한 pxd의 초기 공모전형 이벤트 운영 사례
- [pxd 워크샵과 야유회 문화](concepts/pxd-워크샵과-야유회-문화.md) — pxd 11주년 태안 캠핑 워크샵 사례로 본 요리 경연·팀 미션 기반 야외 행사 구조, 평소 교류가 적던 동료와 자연스럽게 어울리는 정서적 유대 형성 문화
- [pxd 사무실 크리스마스 데코와 연말 문화](concepts/pxd-사무실-크리스마스-데코와-연말-문화.md) — 연말이면 사무실에 크리스마스 트리를 꾸미는 pxd의 반복적 연례 관행, 큰 행사 없이도 계절 데코만으로 정서적 유대·소속감을 형성하는 사례
- [차 문화와 다도](concepts/차-문화와-다도.md) — pxd talks 87 티 스타일링 세미나: 다도의 기원과 음양오행 기반 차 분류, 녹차·홍차·보이차 가공법 차이와 다구 재질별 시음 실습
- [크리스마스 리스 만들기와 플로리스트 공예](concepts/크리스마스-리스-만들기와-플로리스트-공예.md) — pxd talks 79 크리스마스 리스 만들기 세미나: 리스의 3대 기원(결혼식·강림절·장례식)과 소재·제작·보관 규칙
- [핸드드립 커피와 추출 이론](concepts/핸드드립-커피와-추출-이론.md) — pxd talks 81 커피 세미나: 드립 커피의 역사, 드리퍼·드립 서버·드립 포트 도구, 추출 기준표와 나선형 주법
- [해적 부트캠프 신입 교육](concepts/해적-부트캠프-신입-교육.md) — Innovation 3 Group이 운영하는 pxd 신입사원 교육 프로그램, 공방전(공격·방어 팀 논쟁) 형식의 게이미피케이션 기반 '내부혁신가' 양성 커리큘럼
- [히말라야 원정과 팀워크의 교훈](concepts/히말라야-원정과-팀워크의-교훈.md) — pxd talks 80회(김미곤): 히말라야 등반의 도전·디테일·팀워크·판단력 네 화두로 본 극한 상황 속 협업과 책임 있는 의사결정
- [그린그린 업무 대화 프레임워크](concepts/그린그린-업무-대화-프레임워크.md) — 문제를 키우는 레드레드와 함께 다루는 그린그린 대화의 구분, AI가 완성해 보이는 결과물을 미완성이라 말할 책임이 필요해진 AI 시대의 적용

## 경영과 조직
- [Good to Great와 위대한 기업의 원칙](concepts/Good-to-Great와-위대한-기업의-원칙.md) — 짐 콜린스의 Good to Great: 단계5 리더십·사람 먼저·고슴도치 개념·규율의 문화 등 도약 기업의 8가지 원칙
- [OKR 목표 관리 방법론](concepts/OKR-목표-관리-방법론.md) — Objective·Key Result 구조로 야심적 목표를 공격형으로 설정하는 목표관리 방법론 — 구성원 자율성·실패 허용·큰 성과 창출의 원리
- [Purple Cow와 리마커블 전략](concepts/Purple-Cow와-리마커블-전략.md) — Seth Godin의 보랏빛 소가 온다: 평범함은 위험하고, 무시할 수 없을 만큼 다른 것만이 살아남는다는 리마커블 전략
- [그로스 해킹과 데이터 드리븐 조직문화](concepts/그로스-해킹과-데이터-드리븐-조직문화.md) — 데이터 리터러시를 조직 프로세스와 문화로 정착시키는 그로스 해킹 방법론과 웹3 환경의 한계
- [디자인 팀 빌딩과 협업 문화](concepts/디자인-팀-빌딩과-협업-문화.md) — 프로토파이 디자인팀의 파티(Party) 조직 구조와 오피스 아워·디자인 바이 위클리·해피니스 타임 등 팀 빌딩 문화 장치
- [디지털 트랜스포메이션과 조직 문화](concepts/디지털-트랜스포메이션과-조직-문화.md) — 제럴드 케인의 디지털 문화 4대 특성(애자일·실험적·리스크관용·협력적)과 고객 접점·핵심 역량 관점에서 본 조직의 디지털 전환
- [산업의 본질 변화와 미래 예측](concepts/산업의-본질-변화와-미래-예측.md) — 산업의 미래를 주도하는 기업은 그 산업의 본질 변화를 먼저 읽어낸 기업, 전화기·자동차 산업 사례로 본 본질 재정의 관점
- [스포티파이 모델 애자일 조직구조](concepts/스포티파이-모델-애자일-조직구조.md) — Squad·Tribe·Alliance 3단계 분산 자율 조직과 Mixed Methods UX 리서치 운영 방식
- [애자일 조직 전환](concepts/애자일-조직-전환.md) — Shu-Ha-Ri 학습 단계와 인간적 요소 중심의 애자일 전환, SI 프로젝트(Pure/Hybrid Agile)·자동차 산업(임베디드 시스템) 적용 사례
- [전설의 리더, 보와 팀 리더십 원칙](concepts/전설의-리더-보와-팀-리더십-원칙.md) — 미시간대 풋볼 감독 보 스캠베클러의 리더십 7원칙(팀 우선·원칙의 조기 확립·기본기·개인에 대한 관심·위기 시 결단 등)
- [지적자본론과 TSUTAYA 공간 철학](concepts/지적자본론과-TSUTAYA-공간-철학.md) — 마스다 무네아키의 지적자본 개념과 지적인 인재 3자질(디자인 감각·큐레이팅 기술·사명감), TSUTAYA·다케오시립도서관의 지역성·주제 제안형 진열 공간 운영 철학
- [지휘자 리더십 스타일](concepts/지휘자-리더십-스타일.md) — 이타이 탈감의 TED 강연으로 본 지휘자별 상반된 리더십 스타일과 파트너십, 권위 없이 자율적 해석 여지를 남기는 이상적 리더십
- [책장 동기화하기와 디자이너의 젠더 감수성](concepts/책장-동기화하기와-디자이너의-젠더-감수성.md) — 디자이너 김린의 페미니즘 이동식 서가 프로젝트와 pxd 구성원 7인의 1:1 책 추천 워크숍으로 본 UX 디자이너의 젠더 감수성
- [초난감 기업의 조건과 IT 업계 실패 사례](concepts/초난감-기업의-조건과-IT-업계-실패-사례.md) — 릭 채프먼의 『In Search of Stupidity』: 성공 공식을 찾는 경영서 장르 비판, 멍청한 짓을 하지 않는 것과 경쟁자의 실수(행운)가 성공의 실질적 조건이라는 역발상
- [플랫폼 비즈니스와 네트워크 효과](concepts/플랫폼-비즈니스와-네트워크-효과.md) — 마셜 밴 앨스타인 외 『플랫폼 레볼루션』: 파이프라인 vs 플랫폼 구조, 우버·에어비앤비의 긍정적 네트워크 효과 선순환과 신뢰성·생산자-소비자 역할 전환
- [해외 대학생 pxd 방문 교류](concepts/해외-대학생-pxd-방문-교류.md) — 덴버대학교 산업디자인 전공 학생들이 신진섭 교수 인솔로 매년 pxd를 방문하는 연례 교류 프로그램, 디자인 프로세스·조직 문화 소개와 자유 Q&A
- [ESG 경영과 지속가능성](concepts/ESG-경영과-지속가능성.md) — 환경·사회·지배구조(ESG)의 정의와 CSR/CSV와의 차이, 투자자·소비자·벤처 시장에서 ESG가 부상한 배경과 그린워싱 경계
- [물질-관념 이분법과 상징 소비](concepts/물질-관념-이분법과-상징-소비.md) — 홍기빈 소장의 pxd talks 강연: 희소성의 법칙·한계효용 체감의 법칙이 전제한 물질적 욕망의 유한성 논리, 물질재-정신재 경계 붕괴와 상징·기호로서의 소비, 현대 욕망의 근원인 '변신(metamorphosis)' 욕구
- [Game Changer와 고객 중심 혁신](concepts/Game-Changer와-고객-중심-혁신.md) — A.G. 라피와 램 차란의 P&G 혁신 경영서: 고객 중심 혁신의 8가지 동력, Joanne 카드보드 퍼소나와 Living It 몰입형 리서치로 본 게임 체인저 전략
- [디자인적 경영 전략](concepts/디자인적-경영-전략.md) — 에린 조의 『아웃런』: 경영 전략에 디자인 프로세스를 적용하는 접근, 코닥·뉴 코크의 확증 편향 실패 사례와 H형 인재·의미 있는 단순함

## 디자인과 예술
- [Euro IA 2013 해외연수와 OBA 도서관](concepts/Euro-IA-2013-해외연수와-OBA-도서관.md) — 2013년 에든버러 Euro IA 컨퍼런스 참가 후 방문한 암스테르담 공공도서관 OBA 사례로 본 조명·타이포그래피 기반 공간 경험 설계
- [Ex-formation](concepts/Ex-formation.md) — 하라 켄야가 창안한 개념, Information의 반대말로 안다고 믿던 것의 무지를 자각하게 하는 디자인 접근법
- [그래픽 디자인 스튜디오 전시 관람과 영감](concepts/그래픽-디자인-스튜디오-전시-관람과-영감.md) — 그래픽 디자인 매거진 겸 스튜디오 GRAPHIC PROPAGANDA PRESS의 'GRAPHIC 2009 Annual Report' 전시 관람기, 같은 업계 동료 스튜디오 작업에서 얻는 '동종 업계 영감' 사례
- [그림책 디자인과 시각 스토리텔링](concepts/그림책-디자인과-시각-스토리텔링.md) — 이수지 작가의 시각 스토리텔링 철학: 구멍 많은 이야기, 글 없는 그림책, 경계를 허무는 형식 실험
- [디자인 다큐멘터리 Objectified](concepts/디자인-다큐멘터리-Objectified.md) — 게리 허스트윗 감독의 디자인 3부작 2편, IDEO·애플·MUJI·디터 람스 등 인터뷰로 본 "프로토타이핑이 디자인 객관성을 입증한다"는 관점
- [런던 디자인 페스티벌과 전시 트렌드](concepts/런던-디자인-페스티벌과-전시-트렌드.md) — 2013·2019 런던 디자인 페스티벌(100% Design·디자인 정션·텐트런던·London Design Fair) 참관기로 본 지속가능성·바이오머티리얼 화두, 테라조 소재 유행, 국가별 디자인 큐레이션
- [메종오브제 국제 인테리어 박람회](concepts/메종오브제-국제-인테리어-박람회.md) — 파리 노르 빌팽트에서 열리는 세계 최대 인테리어·소비재 박람회, 2013년 'Illuminations·Psychotropia·Funtasy' 테마로 본 신소재·용도 전환 트렌드
- [모션 그래픽 애니메이션과 영감 큐레이션](concepts/모션-그래픽-애니메이션과-영감-큐레이션.md) — pxd 블로그 '영감 동영상' 시리즈가 소개한 런던 애니메이션 스튜디오 Animade의 2D 캐릭터 모션 그래픽 작업(Cowboy's Cat·Fingerprint Art·Jangle Bells 등)
- [미디어아트 인터랙티브아트와 영감 큐레이션](concepts/미디어아트-인터랙티브아트와-영감-큐레이션.md) — pxd 블로그 '영감 동영상' 시리즈의 미디어 아트 편, Pool Party·문준용의 Augmented Shadow·IRIS·Rain Room·Woodland Wiggle 5편 큐레이션
- [비엔나 디자인 위크와 업사이클링 디자인](concepts/비엔나-디자인-위크와-업사이클링-디자인.md) — 2013년 비엔나 디자인 위크 참관기: SECOND LIFE 유리 업사이클링·Gabarage·Nomadic Furniture 3.0·BuzziSpace 어쿠스틱 가구와 훈데르트바서의 곡선 철학
- [스톱모션 애니메이션과 영감 큐레이션](concepts/스톱모션-애니메이션과-영감-큐레이션.md) — pxd 블로그 '영감 동영상' 시리즈로 소개된 스톱모션 작품 4편(The Extra Small·Revolution 등), 종이·팝업북 등 물리적 수작업 질감이 주는 디지털과 구별되는 창작 영감
- [시네마그래프와 디자인 영감 큐레이션](concepts/시네마그래프와-디자인-영감-큐레이션.md) — 사진과 동영상의 중간 형식인 시네마그래프 GIF와 핀터레스트 기반 디자인 영감 탐색 패턴
- [애니메이션 제작과 레이아웃 설계](concepts/애니메이션-제작과-레이아웃-설계.md) — 스튜디오 지브리 레이아웃展으로 본 애니메이션 장면 설계 공정, 분업 환경에서 통일성 유지를 위한 레이아웃의 역할
- [영상 음악 기획](concepts/영상-음악-기획.md) — 영상 음악의 역사와 기능, 스포팅 협업 과정, Cue/Theme/디제시스 사운드 개념, 물리적·심리적·기술적 기능 분류
- [유명인 큐레이터와 비전문가 전시 기획](concepts/유명인-큐레이터와-비전문가-전시-기획.md) — 샤킬 오닐의 'Size Does Matter' 그룹전 큐레이팅 사례로 본, 신체적 경험을 전시 주제로 전환한 비전문가 큐레이션
- [인터랙션 프로모션 영상과 영감 큐레이션](concepts/인터랙션-프로모션-영상과-영감-큐레이션.md) — pxd 블로그 '영감 동영상' 시리즈가 소개한 UI 인터랙션 프로모션 비디오·데모 4편(Spatially Aware Devices·Reader·Readability·Comma)
- [인터랙티브 미디어 아트와 STEAM 교육](concepts/인터랙티브-미디어-아트와-STEAM-교육.md) — 미디어 아티스트 문준용의 인터랙티브 설치 작업(VJing·탠저블 인터페이스·서사형 인터랙션)과 과학·기술·공학·예술·수학을 잇는 STEAM 교육 사례
- [타 장르 아티스트 작품과 영감 큐레이션](concepts/타-장르-아티스트-작품과-영감-큐레이션.md) — 브라운아이드소울 나얼의 그림 작업 소개로 본, 디자인계 밖 아티스트의 작업 방식·소재에서 영감을 얻는 pxd 블로그 캐주얼 큐레이션 문화
- [파브리카와 베네통 커뮤니케이션 전략](concepts/파브리카와-베네통-커뮤니케이션-전략.md) — 베네통 산하 창작 리서치 센터 FABRICA와 올리비에로 토스카니의 논쟁적 이미지 전략, COLORS Magazine·UNHATE 캠페인으로 본 가치 중심 브랜드 커뮤니케이션
- [현대미술 설치작가와 전시](concepts/현대미술-설치작가와-전시.md) — 아니쉬 카푸어 리움 전시로 본 현대 설치 미술의 물성·반사·압도감, 제프 쿤스·펠릭스 곤잘레스-토레스와의 비교

## 디자인 산업
- [Favorite Medium과 글로벌 원격 근무 문화](concepts/Favorite-Medium과-글로벌-원격-근무-문화.md) — 한국·싱가포르·일본·미국 4개국 오피스를 둔 디지털 프로덕트 랩 Favorite Medium의 pxd talks 85회 발표, 슬랙·G-Suite·자체 툴 SIXPAQ 기반 글로벌 원격 협업 방식
- [KIDP Design Driven Innovation 세미나](concepts/KIDP-Design-Driven-Innovation-세미나.md) — 한국디자인진흥원(KIDP)-WDO 이사진 초청 세미나: 멕시코·터키 디자인 산업의 문화적 자본과 정책 대비, 삼성 디자인 철학 'Be Bold, Resonate with Soul'과 One UI
- [UX 얼라이언스 글로벌 네트워크](concepts/UX-얼라이언스-글로벌-네트워크.md) — 26개국 독립 UX 컨설팅사 네트워크 UXalliance, pxd의 2018년 대한민국 대표 회원사 가입과 국내외 양방향 사용자 조사 협업
- [구글 UX 조직문화와 디자인 프로세스](concepts/구글-UX-조직문화와-디자인-프로세스.md) — pxd talks 49, 전 Google 디자이너 김수의 강연: **'Super Flat'** 조직문화, UX University·Kudos 등 내부 툴, PM/디자이너 평가 기준과 디자인 스프린트 관행
- [글로벌 융합기술적용 디자인 워크숍](concepts/글로벌-융합기술적용-디자인-워크숍.md) — 한국디자인진흥원(KIDP)-영국 Engine 협력 서비스디자인 교육 프로그램, 2014년 pxd 국내 파트너 참여와 한국도로공사 요금소·휴게소 경험디자인 과제, 여주휴게소 시범 설치 성과
- [디자인 에이전시와 인하우스 디자인](concepts/디자인-에이전시와-인하우스-디자인.md) — 디자인 중요성 증가 → 인하우스 전환 → 에이전시 위축의 역설, 미국 디지털 에이전시 3물결과 한국 현황
- [디자인 융합 세미나 (2014)](concepts/디자인-융합-세미나-2014.md) — 한국디자인진흥원 '디자인, 융합의 가치' 세미나: 매개자에서 편곡자로 진화하는 디자인 컨버전스, 삼성병원 응급실 서비스 디자인·여수/김제 커뮤니티 디자인·ICT 유아 완구 헤네스 브룬 사례
- [디자이너 창업과 스타트업](concepts/디자이너-창업과-스타트업.md) — Evan Sharp·김봉진 등 디자이너 창업 사례와 Designer Fund, 에이전시 스핀오프로 이어지는 디자인 산업 선순환 구조
- [미국 스타트업 디자이너 업무 문화](concepts/미국-스타트업-디자이너-업무-문화.md) — 야후 시니어 UX디자이너 송민승의 ZocDoc·Yahoo News Digest·Yahoo Aviate 경험으로 본 미국 스타트업 vs 한국 대기업의 의사결정 속도·호칭 문화·리서치 방식 차이
- [스웨덴 디자인 에이전시와 업무 문화](concepts/스웨덴-디자인-에이전시와-업무-문화.md) — Above 에이전시 사례로 본 사람 중심·의도적 축소·지속가능성 원칙, 디자인 테크놀로지스트 직무, 프로젝트 베이스 조직 문화
- [중국 IT 대기업 문화와 디자인 트렌드](concepts/중국-IT-대기업-문화와-디자인-트렌드.md) — BAT(바이두·알리바바·텐센트) 성장 배경과 광군제·위챗 수치로 본 중국 IT 성장 4요인, 관리자/전문가 이원화 커리어 트랙 등 중국 IT 대기업 조직 문화
- [한국 디자인 산업 구조와 디자이너 처우](concepts/한국-디자인-산업-구조와-디자이너-처우.md) — 공급 과잉·R&D 부재·에이전시 영세화의 구조적 순환 고리로 본 한국 디자이너 저임금 문제
- [디자인 어워드와 수상 사례](concepts/디자인-어워드와-수상-사례.md) — Red Dot Best of the Best 기준과 pxd Bedside Station 수상 경험으로 본 국제 디자인 어워드의 전략적 가치

## 접근성과 보조기술
- [VoiceOver와 모바일 스크린리더 UX](concepts/VoiceOver와-모바일-스크린리더-UX.md) — iPhone VoiceOver 스크린 리더의 훑기-포커싱·더블탭 실행 조작 모델을 직접 눈 감고 체험한 사용기, 이름("손쉬운 사용")과 실제 작동의 불일치가 만드는 사용자 혼란
- [보완대체 의사소통 AAC](concepts/보완대체-의사소통-AAC.md) — 언어 표현 장애를 보완·대체하는 의사소통 수단, My First AAC·마이토키 국내 사례

## 기술과 사회
- [UI와 디바이스 역사](concepts/UI와-디바이스-역사.md) — 마우스·QWERTY·VisiCalc·플래시-매틱 등 UI 요소의 최초 순간들
- [pxd 특허와 스마트폰 UI 선행 사례](concepts/pxd-특허와-스마트폰-UI-선행-사례.md) — 2004~2005년 SK텔레콤과 함께 출원한 전화번호 하이퍼링크·앱 상태 저장·애넌시에이터·스핀홈 특허, mMessenger 스레드 UI 특허가 아이폰(2007)보다 선행한 사례
- [디자인 특허 전략](concepts/디자인-특허-전략.md) — 특허·디자인권·상표권 등 다섯 범위로 아이디어를 보호하는 전략, 애플-삼성 특허전쟁 사례로 본 선점의 중요성과 중소기업 실무 팁
- [디지털 방송 위젯 시스템 특허](concepts/디지털-방송-위젯-시스템-특허.md) — pxd가 제안해 2010년 등록된 디지털 방송 위젯 시스템 및 위젯 출력 방법 특허, 방송 화면을 부가 정보 플랫폼으로 확장하려던 스마트TV 이전 시기의 시도
- [패널 해상도 표준과 디자인 리소스 큐레이션](concepts/패널-해상도-표준과-디자인-리소스-큐레이션.md) — 반응형 웹 이전 시대 NTSC·PAL·VGA·XGA·HD 등 패널 해상도 표준 참고 관행과 pxd 블로그 "자료창고" 카테고리의 리소스 큐레이션 성격
- [Adobe CS5 신기능 리뷰 영상 공유](concepts/Adobe-CS5-신기능-리뷰-영상-공유.md) — Adobe CS5 출시 직후 콘텐츠 인식 채우기·퍼스펙티브 편집·3D 기능을 유튜브 리뷰 영상 링크로 모아 소개한 자료창고형 게시물
- [레티나 디스플레이와 고해상도 UI](concepts/레티나-디스플레이와-고해상도-UI.md) — 뉴아이패드 레티나 디스플레이 후기로 본 고해상도 화면의 체감 효과, PDF/epub 가독성, 이미지 리샘플링 문제
- [컴퓨터 케이스 커스터마이징](concepts/컴퓨터-케이스-커스터마이징.md) — 기능과 표현의 분리, 산업 디자인과 GUI 작업에 공통된 접근 방식
- [MS KIN 폰과 SNS 특화 디바이스](concepts/MS-KIN-폰과-SNS-특화-디바이스.md) — 2010년 MS 첫 자체 브랜드 폰 KIN, 앱·확장성 대신 SNS 소통이라는 단일 컨셉에 기능을 집중시킨 니치 디바이스 설계 사례
- [Entourage Edge와 듀얼스크린 전자책 리더기](concepts/Entourage-Edge와-듀얼스크린-전자책-리더기.md) — CES 2010 공개 e-ink+LCD 듀얼스크린 하이브리드 리더기 리뷰, 콘텐츠 확보가 하드웨어 폼팩터보다 시장 성패를 좌우한다는 진단
- [3D 프린팅 기술](concepts/3D-프린팅-기술.md) — FDM·SLA·SLS 방식의 3D 프린팅 기초, RepRap 오픈 소스와 제조 민주화
- [3D 애니메이션과 렌더링 기술](concepts/3D-애니메이션과-렌더링-기술.md) — 빅 히어로 제작 비하인드: 하이페리온 렌더링 엔진과 소프트 로보틱스 기반 캐릭터 설계
- [VR 산업 동향과 기술 트렌드](concepts/VR-산업-동향과-기술-트렌드.md) — 2016년 VR 미니컨퍼런스로 본 VC의 3대 우려(Confusing·Competing·Collaborating), 하드웨어·콘텐츠·모바일 모션트래킹 초기 단계 현황과 대형 플랫폼사 참여가 가를 표준화 변곡점
- [USIM 보안](concepts/USIM-보안.md) — AKA 프로토콜 기반 USIM 인증 구조, SKT 해킹 사건으로 본 유심 보안 위협과 대응
- [마켓 5.0과 디지털 전환](concepts/마켓5.0과-디지털-전환.md) — 필립 코틀러의 마켓 5.0 프레임워크와 나이키·넷플릭스·힐튼·아마존의 디지털 전환 전략
- [모바일 메가트렌드 2010 세미나 리뷰](concepts/모바일-메가트렌드-2010-세미나-리뷰.md) — 삼성전자 장동훈 전무의 2010년 서울대 세미나 강연: 터치폰·앱스토어·오픈OS 부상과 2010~2011년 5대 메가트렌드, 애플 대비 삼성의 에코시스템 대응 자평
- [모바일 OS 춘추전국시대](concepts/모바일-OS-춘추전국시대.md) — 2010년 MWC 인텔 모블린+노키아 마에모의 미고(MeeGo) 제휴로 본 윈도폰7·바다·심비안·안드로이드·iOS 등이 난립하던 스마트폰 OS 파편화 스냅샷
- [건축과 융합적 인간](concepts/건축과-융합적-인간.md) — 김정후 박사 강연으로 본 융합(convergent)과 분화(divergent), 알도 로시·미스 반 데 로에·리트벨트·알바 알토·노먼 포스터의 건축 원리와 르네상스형 인간
- [기술 인간화와 디자인 윤리](concepts/기술-인간화와-디자인-윤리.md) — 필립 K. 딕 소설을 통해 본 인간다움의 세 요소(감정이입·개별자성·생명창조)와 AI 편향·디지털 소외를 포함한 UX 설계자의 윤리 책임
- [소셜 미디어와 기술 윤리](concepts/소셜-미디어와-기술-윤리.md) — '소셜 딜레마' 다큐멘터리를 통해 본 기술 윤리 문제
- [소셜 미디어 진화와 사회적 이슈](concepts/소셜-미디어-진화와-사회적-이슈.md) — 2012년 pxd talks 06(한상기 박사): 싸이월드→페이스북·트위터·핀터레스트 SNS 진화와 관계 맺기 다양성·아이덴티티 매니징·인터넷 자경주의·프라이버시·표현의 자유 5대 사회적 논제
- [언택트와 온택트](concepts/언택트와-온택트.md) — 대면 서비스를 대체하는 1:1 절차적 인터랙션의 언택트와, 물리적 거리를 유지하며 디지털로 연결하는 1:n·n:n 실시간 인터랙션의 온택트, Pine의 경험경제 분류(Absorbed/Immersive)로 본 차이
- [영상 인식 기반 세컨드 스크린 서비스](concepts/영상-인식-기반-세컨드-스크린-서비스.md) — 이미디오(엔써즈)의 이미지 기반 영상 인식 기술과 미리보기 우회 꼼수, 메인 화면을 방해하지 않는 세컨드 스크린이 스마트TV의 핵심 가치라는 통찰
- [Smart TV UX와 Connected TV 생태계](concepts/Smart-TV-UX와-Connected-TV-생태계.md) — pxd talks 41(전원근) 강연: 인터랙티브 TV·PVR 실패사를 혁신기업의 딜레마(종속적/파괴적 혁신)로 설명, OTT 부상과 파편화 문제로 본 Connected TV 생태계
- [SNS 드라마와 소셜 비디오 콘텐츠](concepts/SNS-드라마와-소셜-비디오-콘텐츠.md) — 2013년 무렵 SNS 플랫폼 기반으로 확산된 모바일 최적화 단편 드라마 콘텐츠, 모바일 최적화·소통과 공유·브랜디드 콘텐츠 3가지 특징
- [우분투 OS 디자인 철학](concepts/우분투-OS-디자인-철학.md) — 데비안 기반 리눅스 배포판 우분투의 디자인 철학
- [디자인 개념의 역사적 변천](concepts/디자인-개념의-역사적-변천.md) — 카타리나 베렌츠의 『디자인 소사』로 본 '디세뇨'에서 크리스토퍼 드레서·루이스 설리번·발터 그로피우스·레이먼드 로위(MAYA 원칙)·헨리 드레이퍼스·빅터 파파넥까지 이어진 디자인 개념 확장사
- [디자인과 삶의 철학](concepts/디자인과-삶의-철학.md) — 서울대 김민수 교수의 '디자인 역사 산책' 강의: "I see you" 태도로 본 디자인 역사, 디터 람스·BRAUN의 철학 있는 디자인 사례
- [독일 바우하우스와 근대성](concepts/독일-바우하우스와-근대성.md) — '디자인 역사 산책' 2주차: 전후 독일 사회를 수습하려 한 그로피우스의 바우하우스, 총체 예술 지향과 커튼월 공법의 데사우 校舍
- [이상(李箱)과 융합예술](concepts/이상과-융합예술.md) — '디자인 역사 산책' 3주차: 문학가 이상을 건축가·타이포그래피 디자이너를 겸한 융합예술가로 재조명, 서촌 유년 체험과 작품의 관계
- [이탈리아 디자인과 인본주의](concepts/이탈리아-디자인과-인본주의.md) — 인본주의 철학을 바탕으로 한 이탈리아 디자인의 역사: 피닌파리나·올리베티·소트사스·멤피스 그룹
- [일본 디자인의 역사와 디자이너들](concepts/일본-디자인의-역사와-디자이너들.md) — 자포니즘·화혼양재·모더니즘 두 축으로 본 일본 디자인 역사: 가메쿠라 유사쿠·무라카미 다카시 등
- [한국 디자인의 역사와 과제들](concepts/한국-디자인의-역사와-과제들.md) — '디자인 역사 산책' 6주차(마지막): "디자인은 부분의 미학이 아닌 삶 전체의 총체적 미학이어야 한다"는 결론과 디터 람스·브라운으로 대표되는 소신·철학의 축적
- [죽음의 경험 디자인](concepts/죽음의-경험-디자인.md) — 하이데거의 실존적/회피적 죽음 태도와 케이틀린 도티·퀴블러 로스를 통해 본 장례·임종 경험 디자인의 철학적 질문
- [헨리 드레이퍼스와 토탈 디자인](concepts/헨리-드레이퍼스와-토탈-디자인.md) — 인간공학 기반 산업 디자인의 원형, 전화기 3부작과 Honeywell 온도조절계·Nest 특허분쟁, '토털 디자인' 개념
- [넛지와 다크 패턴](concepts/넛지와-다크-패턴.md) — 부드러운 유도인 넛지와, 사용자를 속여 이익을 취하는 다크 패턴 5대 유형(미끼와 스위치·위장 광고 등)의 경계와 디자이너의 윤리적 책임
- [대한민국 HCI-UI-UX 역사](concepts/대한민국-HCI-UX-역사.md) — 1990년 HCI 학회부터 2010년대까지 한국 UI/UX 업계의 연표
- [CHI 2015 학회와 HCI 연구 트렌드](concepts/CHI-2015-학회와-HCI-연구-트렌드.md) — 게임 몰입·NUI·EMS 촉각 내비게이션·웨어러블 사변적 디자인 등 CHI 2015 국제 학회 Paper 세션에서 발표된 HCI 연구 트렌드 종합
- [Siggraph 2014 컨퍼런스와 웨어러블 컴퓨팅](concepts/Siggraph-2014-컨퍼런스와-웨어러블-컴퓨팅.md) — 웨어러블 컴퓨팅 역사(1968 HMD·스티브 만·Touring Machine)와 기술 분류, 디즈니 DisneyQuest 사례, Mark Billinghurst의 웨어러블 인터페이스 디자인 가이드라인
- [디지털 헬스케어 혁신 사례](concepts/디지털-헬스케어-혁신-사례.md) — pxd talks 44(최윤섭) 강연: 개인 유전체 분석(Foundation Medicine·23andMe), Google Glass·Scanadu·Proteus 등 디지털 헬스케어 기기, AliveCor 웨어러블 심전도, PatientsLikeMe 환자 커뮤니티로 본 헬스케어-IT 컨버전스 혁신 사례
- [홈서버 원격 제어](concepts/홈서버-원격-제어.md) — WOL·Air Video·VNC를 조합해 스마트폰만으로 PC 전원부터 파일 감상·종료까지 처리한 2010년 개인 홈서버 구축 사례, 스마트폰-PC 중간 디바이스 레이아웃 표준의 부재
- [클라우드 컴퓨팅](concepts/클라우드-컴퓨팅.md) — 클라우드의 어원과 SaaS·PaaS·IaaS 3층 구조, 세일즈포스닷컴 채터와 퍼스널 클라우드 서비스(PCC)로 본 N스크린 전략
- [인사이트 노동자](concepts/인사이트-노동자.md) — 정지훈 『무엇이 세상을 바꿀 것인가』 독후감: BCG 리치 레서가 제시한 개념, 지식 노동 자동화 이후 비판적 사고·공감을 담당하는 미래 노동자상
- [애드몹 리포트와 안드로이드 시장점유율 오독](concepts/애드몹-리포트와-안드로이드-시장점유율-오독.md) — 2010년 AdMob 리포트의 "share of requests"를 마켓쉐어로 오독해 안드로이드가 아이폰을 추월했다고 확산된 사례로 본 이차 자료 통계 오독
